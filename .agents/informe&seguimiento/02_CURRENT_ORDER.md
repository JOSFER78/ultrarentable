...
- **Bounded evolution is diversity-preserving:** when the daemon asks for only 4 children, selection takes deterministic representatives across structural/regime, exit/risk, session/complexity and entry-dynamics groups instead of consuming the first four mutations.
- **No-op mutations are discarded before budgeting:** a mutation that leaves all executable inputs unchanged cannot consume an evolution slot.
- StrategyResearchLoop evaluates ULTRA hypotheses through `FastEngineAdapter -> CanonicalCompiler -> UniversalDeterministicBacktestEngine`, keeping the optimization path on the canonical execution stack.
- **FONDEO now has its own canonical research path**: `FundingResearchLoop + FundingEvolutionEngine`, using fixed-point stop/target, bounded risk-per-trade, explicit time-stop/session mutations and the same canonical adapter/backtest chain. This prevents FONDEO candidates from accidentally being optimized as ULTRA strategies.
- Candidate frontier selection is family-stratified for ULTRA: one best candidate per executable archetype is retained before filling remaining survivor slots by score. This prevents one family from monopolizing the next generation.
...
- Regression tests cover semantic mutation effects, small-budget mutation diversity, semantic no-op suppression, family-stratified survivor selection, FONDEO executable session mutations, structural filters, score behavior, future-data rejection and temporal-shift preservation.
...
1. Run the expanded structural-evolution campaign on real persisted ULTRA datasets.
2. Run the route-specific FONDEO evolutionary campaign only where a complete physical dataset and canonical cost model exist.
3. Compare mutation families by validation stability and failure mode, not only top score.
4. Expand the real-data universe where a canonical venue/data source provides complete history.
5. Re-run final blind OOS only for frozen family-level champions after research selection is complete.
6. Promote only complete evidence-backed candidates.
