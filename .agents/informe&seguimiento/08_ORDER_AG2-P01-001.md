...
This order is the single active order after external review of AG2-P00-002.
...
Antigravity may inspect the full repository to understand dependencies, but it may **modify only Phase 01 scope** and direct dependencies strictly required to make Phase 01 reproducible.

Do NOT:

- start Discovery Factory;
- build Strategy Genome/clustering;
- optimize strategies;
- build Meta-Strategy Lab;
- implement FONDEO research logic;
- implement ULTRA +1000% research;
- redesign unrelated UI;
- clean unrelated technical debt.

Any finding outside Phase 01 must be reported as `DEFERRED_TO_FUTURE_ORDER` and left untouched unless proven a direct blocker for the Phase 01 data chain.
...
Create/repair the canonical registry so that assets and timeframes are registry-driven rather than hardcoded in the general engine.
