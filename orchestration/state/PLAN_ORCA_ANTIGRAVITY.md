...
| **2. Sin commit** | Hook `pre-commit` en **lista blanca**: NADIE commitea salvo `ORQ_COMMIT=1`, sea cual sea la rama o el entorno (corregido el 02-09: la versión por lista negra `agy/*`/`AGY_AGENT` estaba muerta en el montaje real, porque Orca nombra las ramas `JOSFER78/agy-<ID>` y no propaga variables al worker; un agente de humo commiteó de verdad). Bloquea además datasets pesados en el índice y borrados sin MANIFEST. Los hooks viven en `C:/Users/yo/.orq-hooks` (`core.hooksPath` absoluta, fuera de todos los worktrees: un agente no puede editar el hook que lo ata) | Los commits "para cerrar" que destruían la revisión de Emilio |
| **3. Sin push** | Hook `pre-push`: bloquea cualquier push sin `ORQ_PUSH=1` y todo push de una rama con `agy` en el nombre (`*agy/*`, `*agy-*`) | Publicar en GitHub por su cuenta |
