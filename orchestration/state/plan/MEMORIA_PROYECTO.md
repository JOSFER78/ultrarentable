...
## Motor de mejora — fase 2 el mismo día: agentes que piensan, programas que ejecutan (2026-09-06 16:50 CEST)

Emilio rechazó las hipótesis hardcodeadas y pidió un debate de agentes por estrategia. Investigado (sí para proponer/criticar, no para medir; el "debate" previo de la API era pregrabado) e implementado: `sqx_hypothesis_debate.py` con dos proponentes ciegos, validación determinista de cada cambio, crítico, árbitro sin consenso forzado y presupuesto de búsqueda; integrado en el ciclo (`dossier` → debate → `prepare-local --hypotheses` → `run` → `evaluate`). Dos debates reales (Claude Opus 5 vía Claude Code, ~2 USD y ~5 min cada uno) y un ciclo 02 en SQX: la hipótesis principal de los agentes (validez de la orden larga 4→8) resultó real pero inerte (`NO_EFFECT_IN_SAMPLE`) y el objetivo intermedio confirmó la advertencia del árbitro (`HISTORICAL_FIT_ONLY`); el registro acumula seis variantes. Endpoint de IA del sistema fijado por Emilio: el omnirouter de Oracle (`/pro/omniroute/api/v1`), alias de tarea `ultrarentable-mejora-*` a definir en el panel de superadmin; `auto` sin créditos en OpenRouter, `auto/best-reasoning` operativo. Pendiente de Emilio: combos en el panel, créditos, vocabulario de filtros de hora/día y datos 2026 de @EW.

## Motor de mejora — fase 5: todas las estrategias extraídas, mejoradas todo lo posible (2026-09-06 21:00 CEST)

Emilio: "la idea es que con las estrategias que tenemos extraídas de SQX, mejorarlas todo lo posible". Inventario en la VPS: 125 estrategias únicas seleccionadas por el generador de fondeo (23 celdas, OOS ≈ 20 %, alias CFD salvo M6E) y 5 entregadas por la fase 5 de triaje; el `ToImprove` de Ultra_Matrix (2 034 AUDUSD con OOS de tres días) y los bancos brutos quedan fuera; el catálogo de la API en la VPS está vacío. Servicio v2: admisión desde esas fuentes con procedencia y sin repetir, reparto entre estrategias por prioridad y atención, presupuesto por experimentos sin progreso, linaje desde variantes aceptadas (la variante recalculada pasa a ser hija; evidencia OOS exigida MODERATE → STRONG con la profundidad; tope 3), temporizador cada 15 min con hasta 6 experimentos en 12 min. Guía en `docs/Laboratorio/07_MOTOR_MEJORA_CICLO.md`; resultado en `orchestration/results/codex/MOTOR_MEJORA_20260906/RESULTADO.md` §12.

## Motor de mejora — fase 4: filtros de hora, día y dirección con semántica comprobada (2026-09-06 20:00 CEST)

Emilio: "dedícate solo a que el motor nuevo funcione; para pruebas usa tu IA, luego el sistema usará otra de omniroute". Los agentes habían pedido en las tres rondas anteriores filtros de horario y de día que el vocabulario no expresaba. Añadidos en `sqx_variant_mutations.py` como bloques nativos de SQX en la regla de entrada (`hour_range`, `exclude_weekdays`, `disable_direction`), con verificación de alcance (solo adiciones dentro del `If` de su regla) y catálogo reajustable; semántica comprobada con dos recálculos de mecanismo en la VPS (`UR_IMPROVE_MECANISMO_FILTROS_01/02`, sin registro): la hora filtrada es la de apertura de la barra que genera la señal y el primer relleno cae en la hora siguiente; la orden pendiente de la víspera puede rellenarse en un día excluido; sin cortos = 140/48 operaciones frente a 173/56. El dosier del debate oculta las tablas OOS por segmento para no ajustar sobre desarrollo. Nueve módulos desplegados con hashes iguales en la VPS; 187 pruebas correctas. Un cambio ajeno en `sqx_trade_diagnosis.py` (calendario hasta la sesión de la tarde del último día, tolerancia de ±3 días) se conserva y las pruebas se adaptan. Ciclo 04 con debate por Claude (mi IA, `claude-cli`, 2,32 $): cinco propuestas, ninguna con filtros, dos recalculadas en la VPS (`REJECTED_WORSE` e `INCONCLUSIVE`); registro con 13 variantes y 9 experimentos, cero candidatas. Hueco siguiente: activar `move_sl_to_be` desde `None` (dos propuestas murieron ahí). El servicio de la VPS sigue con omniroute.

## Motor de mejora — fase 3: programas independientes y servicio autónomo (2026-09-06 17:30 CEST)

A petición de Emilio ("agentes en el sistema, todo como módulos independientes"): la política de evaluación se separó en `sqx_variant_evaluation.py`, la biblioteca fija quedó como andamio de pruebas, y `sqx_improvement_service.py` (unidades `sqx-mejora-agentes.service/.timer`, cada hora) encadena en la VPS inbox → contrato → órdenes base → diagnóstico → debate por omnirouter → variantes verificadas → recálculo SQX → evaluación → registro → entrega, con cola persistente, presupuesto por estrategia, reintentos limitados y estados con diagnóstico (`queue.json`, `status.json`, `outbox/`). Mapa de módulos y contratos en `docs/Laboratorio/07_MOTOR_MEJORA_CICLO.md`. El temporizador antiguo (`sqx-improvement.timer`, recetas MYM/MNQ) sigue; ambos se excluyen por la reclamación del motor.
