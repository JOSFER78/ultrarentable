...
| **W2.8** | **La telemetría no persiste PF/ops de IS de quien SUPERA IS** (I2 §2.1.b): solo registra la etapa de muerte. Sin eso los near-misses no se pueden caracterizar en las tres etapas. Ampliar el registro por candidato con `is_pf`, `is_trades`, `val_pf`, `val_trades` cuando existan; encaja con W2.7 | SUB | c106/c107/c108 aparecen con PF de IS y VAL en el JSON del embudo |

| **W2.9** | **Motor 5.18.0 — sesiones con DST y ventana por familia (regla #26; decisión D10)**. FORENSE (`results/forense_familias_ES15m.md`, reproducido por el ORQ): la sesión RTH está fija en 13:30-20:00 UTC todo el año y la apertura real es 14:30 UTC en invierno (33,4 % de los días); además `funding_discovery` adjunta la ventana RTH a las 6 familias, asfixiando `SQUEEZE_BREAKOUT` (83,5 % de sus rupturas ocurren 00-08 UTC). Contrato completo en `state/contratos/W29_motor_5_18_sesiones_dst.md`: hora local de mercado + `zoneinfo` por vela; familias A/B/D con ventana Globex y flat 15:10 CT (Topstep, la única verificada), NO `None`; bump 5.18.0 + baseline F02 nuevo con diff explicado celda a celda (9 ULTRA idénticas). **Consecuencia: E1/E2 se repiten con 5.18.0 (E2b) antes de concluir nada sobre las familias de sesión** | SUB (Sonnet) cuando haya hueco en el semáforo | aceptación §4 del contrato, re-ejecutada por el ORQ |
...
| W4.2 | **HECHO (AG-C, 01-09; I3 §2.5 confirma que meta_ensemble y meta_strategy_pipeline ya leían el SSOT)**. Eliminar hardcode `engine_version 5.4.0` en fast_engine_adapter, funding/strategy_research_loop, excel_master_catalog → leer `services/engine_version.py`. Residuo consciente, NO es hardcode de motor: `engine_version.py:95-100` define `CURRENT_PIPELINE_VERSION`/`*_POLICY_VERSION = "5.4.0"` (versiones de política, no del motor); se revisan cuando exista el registro de gates (W4.3/W4.7) | `grep -rn 5.4.0 services scripts` solo devuelve engine_version.py y su changelog |
...
| **W4.8** | **`POST /api/v1/providers/sync` finge frescura** (M3 §1.3): reescribe `verified_at` a HOY en cada llamada sin consultar ninguna fuente (`providers_router.py:189-215`) y responde "Sincronización completada exitosamente". Fail-closed: el endpoint debe negarse (501 con motivo) o re-verificar de verdad; nunca repintar fechas | SUB | llamar al endpoint no cambia `verified_at` |
| **W4.9** | **Regla #26 pendiente para el catálogo del MOTOR** (M3 §4): `services/exploitation_engines/prop_firm_engine.py::PROP_FIRM_CATALOG` alimenta `PropFirmProfile` del motor con 5 campos sin fuente y con contradicciones frente a I4 (MFFU consistencia 40 vs 50 verificado; Tradeify 30 vs 40; Tradeify Growth con DLL=None). Corregirlos altera qué operaciones cierra el motor ⇒ bump `CURRENT_ENGINE_VERSION` + criterio de identidad propio (las celdas con `prop_profile` SÍ deben cambiar; las 15 de F02 no) | ORQ | decisión escrita campo a campo antes de tocar el motor |
...
| **W5.8** | **Migración de `/prop-firms` al catálogo v2** (M3 §1.3-1.4): `apps/web/lib/prop-firms.ts` (4.307 LOC, 65+ cuentas con cupones/afiliados) no es sustituible 1:1 por el catálogo de 6 firmas de I4 (reglas de riesgo/ToS). Decisión D7: la página muestra el catálogo v2 (reglas verificadas con fuente) y la parte comercial (cupones/afiliados) se retira hasta re-verificarse aparte; `lib/prop-firms.ts` a cuarentena solo entonces | SUB web | cero `import` de `lib/prop-firms` en `apps/web`; `/prop-firms` pinta `SourceRef` por dato |

## W6 — META-FONDEO Y EXAMEN (bloqueado hasta ≥1 certificada; diseño se adelanta)

| id | Tarea | Dueño | Verificación |
| :-- | :--- | :-- | :--- |
| **W6.0** | **Saneamiento previo de `services/portfolio/` (I3 §2, §7; verificado por el ORQ)**: (a) `autonomous_meta_daemon.py:84-98` lee 5 claves que `assemble_meta_portfolio()` ya no devuelve ⇒ `KeyError` en bucle cada 60 s en cuanto haya 2 candidatas (hoy inerte: `ULTRARENTABLE_AUTONOMOUS_RUNTIME` no está definida en el PC y hay 0 candidatas) — se retira del arranque hasta que `services/meta/` lo sustituya; (b) fabricación viva: `portfolio_engine.py:116-127` (10 retornos de 0,01 y matriz identidad inventados; NaN→identidad), `portfolio_combiner.py:83-107` (correlación sobre NIVELES de equity, NaN→0,0, PF techo 99,0), `factory/portfolio_sprint_engine.py:93` (`correlation_score=0.18` literal), `factory/ultra_portfolio_engine.py` (huérfano) ⇒ **cuarentena** con sus tests dedicados (D8); `portfolio_engine` se retira de `services/portfolio/__init__.py`; (c) `meta_ensemble_service.py:138-145` acepta 6 estados legacy que `meta_strategy_pipeline.py:48` no reconoce ⇒ unificar `certified_statuses` en `services/meta/`; (d) **hecho D8 (01-09 22:00)**: 4 módulos + test dedicado en `cuarentena/fabricadores_meta_20260901/`, tests compartidos divididos, `import services.api.app.main` OK, 6/7 tests meta en verde — el que falla (`test_meta_strategy_engine.py::test_assemble_meta_portfolio_from_two_real_backtests_with_distinct_configs`) **ya fallaba antes**: lee una ruta absoluta del VPS (`/home/ubuntu/...`), defecto **W6.0.d**: resolver el dataset por nombre relativo al repo | ORQ+SUB | `python -c "import services.api.app.main"` OK; tests meta vivos en verde; daemon no arranca |
