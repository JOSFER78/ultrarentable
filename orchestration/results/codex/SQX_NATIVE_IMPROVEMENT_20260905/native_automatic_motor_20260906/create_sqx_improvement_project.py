...
def configure_task(root: ET.Element, *, input_databank: str) -> None:
    ...
    # Databank improvement rejects databank-full: it needs a per-source budget.
    stop.set("type", "time-limit")
    ...
    require(entry, "./LongImprovement").set("use", "false")
    require(entry, "./LongImprovement").set("action", "add-or-replace")
    require(entry, "./ShortImprovement").set("use", "false")
...
def main() -> int:
    ...
    parser.add_argument("--candidate-file", type=Path, help="Bind actual source rules and tested market/risk settings")
    ...
    with tempfile.TemporaryDirectory(prefix="ultrarentable-sqx-improve-") as temp_name:
        ...
        if args.candidate_file:
            with zipfile.ZipFile(args.candidate_file) as candidate:
                settings = ET.fromstring(candidate.read("settings.xml"))
                if settings.get("ResultName") != args.candidate:
                    raise ValueError("Candidate archive identity mismatch")
                last = ET.fromstring(candidate.read("lastSettings.xml"))
                require(ET.fromstring(candidate.read("strategy_Portfolio.xml")), "./Strategy")
            task_root = task_tree.getroot()
            for tag in ("Data", "Resources", "Options", "RiskMoneyManagement", "ATMs"):
                replacement = copy.deepcopy(require(last, tag))
                if tag in ("Options", "RiskMoneyManagement"):
                    replacement.set("customSettings", "true")
                task_root.remove(require(task_root, tag))
                task_root.append(replacement)
            # Search at original sizing; independent retest enforces integer contracts.
            # Do not silently change the hypothesis during the search.
            for bank in config_root.findall("./Databanks/Databank"):
                bank.set("syncType", "Auto-sync never")
            old_resources = config_root.find("Resources")
            if old_resources is not None:
                config_root.remove(old_resources)
                config_root.append(copy.deepcopy(require(last, "Resources")))
            config_tree.write(config_path, encoding="utf-8", xml_declaration=False)
    ...
    manifest = {
        ...
        "candidateSha256": sha256(args.candidate_file) if args.candidate_file else None,
        "partsToImprove": ["exit rules"],
        "preserved": ["entry rules", "order types", "market", "costs", "risk", "OOS split"],
        "executionBudget": {"population": 16, "generations": 2, "maxBank": 12, "nativeMinutes": 1,
                            "deadlineRequiresExternalSupervisor": True},
