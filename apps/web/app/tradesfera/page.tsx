    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-24 text-[var(--text-1)]">
      ...
      <div className="bg-[var(--surface-1)] border border-white/[0.08] backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-2xl space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3 max-w-3xl">
            <div className="flex flex-wrap items-center gap-2.5">
              <span className="px-3 py-1 rounded-full text-xs font-mono font-black bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text-2)]">
                TRATADO MAESTRO V2
              </span>
              <span className="text-xs font-mono text-[var(--text-2)]">
                FUTUROS CME · METODOLOGÍA CUANTITATIVA TRADESFERA
              </span>
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-[var(--profit-dim)] border border-[var(--profit)] text-[var(--profit)]">
                167.000€+ AUDITADOS
              </span>
            </div>
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-3 shrink-0">
            <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-xl p-3.5 text-center min-w-[130px]">
              <span className="text-[10px] text-[var(--text-2)] uppercase font-mono block">Tasa Auditada</span>
              <div className="text-2xl font-black text-[var(--profit)] font-mono tabular-nums">26.6%</div>
              <span className="text-[10px] text-[var(--text-3)] block">vs 2.5% industria</span>
            </div>

            <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-xl p-3.5 text-center min-w-[130px]">
              <span className="text-[10px] text-[var(--text-2)] uppercase font-mono block">Corpus en Disco</span>
              <div className="text-2xl font-black text-[var(--text-2)] font-mono tabular-nums">16 Módulos</div>
              <span className="text-[10px] text-[var(--text-3)] block">Dossier integral</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-[var(--border)]">
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/prop-firms"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-mono font-bold bg-[var(--surface-3)] hover:bg-[var(--surface-3)] text-[var(--text-1)] transition shadow-lg "
            >
            ...
            <Link
              href="/trading-desk"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-mono font-bold bg-[var(--profit-dim)] hover:bg-[var(--surface-3)] text-[var(--profit)] border border-[var(--profit)] transition shadow-sm"
            >
            ...
            <Link
              href="/ultra"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-mono font-bold bg-[var(--surface-2)] hover:bg-[var(--surface-3)] text-[var(--text-1)] border border-[var(--border)] transition shadow-sm"
            >
              <Flame className="w-4 h-4 text-[var(--text-2)]" />
              <span>Trading Desk Ultra</span>
            </Link>
          </div>

          {/* Coupon Copy Pill */}
          <div className="flex items-center gap-2 bg-[var(--surface-1)] border border-[var(--border)] rounded-xl px-3 py-1.5">
            <span className="text-[11px] text-[var(--text-2)] font-mono">Cupón Oficial Unificado:</span>
            <button
              onClick={handleCopyCode}
              className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-[var(--surface-2)] text-[var(--text-1)] hover:bg-[var(--surface-2)] font-mono text-xs font-black border border-[var(--border)] transition"
              title="Copiar cupón TRADESFERA"
            >
              {copiedCode ? <Check className="w-3 h-3 text-[var(--profit)]" /> : <Copy className="w-3 h-3 text-[var(--text-2)]" />}
              <span>{copiedCode ? "¡Copiado!" : "TRADESFERA"}</span>
            </button>
            <span className="text-[10px] text-[var(--profit)] font-mono font-bold">(50%-90% OFF)</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-mono font-bold text-[var(--text-2)] tracking-wider uppercase">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Puerta 1 */}
          <div className="bg-[var(--surface-1)] border border-white/[0.08] hover:border-[var(--border)] backdrop-blur-xl rounded-2xl p-5 shadow-lg transition-all space-y-3 group">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text-2)] flex items-center justify-center text-xs font-black font-mono">
                1
              </div>
              <h3 className="text-sm font-bold text-[var(--text-1)] group-hover:text-[var(--text-1)] transition">
                Descuentos Centralizados
              </h3>
            </div>
            <p className="text-xs text-[var(--text-1)] leading-relaxed">
              Convenio unificado con las mejores prop firms de futuros CME con código <code className="text-[var(--text-1)] bg-[var(--surface-2)] px-1 py-0.5 rounded font-mono font-bold">TRADESFERA</code> (50%-90% OFF).
            </p>
            <div className="text-[11px] text-[var(--profit)] font-mono pt-2 border-t border-[var(--border)]">
          ...
          <div className="bg-[var(--surface-1)] border border-white/[0.08] hover:border-[var(--border)] backdrop-blur-xl rounded-2xl p-5 shadow-lg transition-all space-y-3 group">
            ...
            <div className="text-[11px] text-[var(--text-2)] font-mono pt-2 border-t border-[var(--border)]">
          ...
          {/* Puerta 4 */}
          <div className="bg-[var(--surface-1)] border border-white/[0.08] hover:border-[var(--profit)] backdrop-blur-xl rounded-2xl p-5 shadow-lg transition-all space-y-3 group">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-[var(--profit-dim)] border border-[var(--profit)] text-[var(--profit)] flex items-center justify-center text-xs font-black font-mono">
                4
              </div>
              <h3 className="text-sm font-bold text-[var(--text-1)] group-hover:text-[var(--profit)] transition">
      ...
      <div className="bg-[var(--surface-1)] border border-white/[0.08] backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-4">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text-2)] text-xs font-mono font-bold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-xl p-4 space-y-2">
            <label className="text-[11px] font-bold text-[var(--text-2)] uppercase font-mono block">
            ...
            <input
              ...
              className="w-full bg-[var(--bg)] text-[var(--text-1)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm font-bold font-mono focus:border-[var(--border)] focus:outline-none"
            />
            <span className="text-[10px] text-[var(--text-3)] block font-mono">
        ...
        {/* Results Bar */}
        <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-2xl p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="space-y-1">
            <span className="text-[10px] text-[var(--text-2)] uppercase font-mono block">Disparos / Balas (N)</span>
            <div className="text-2xl font-black text-[var(--text-1)] font-mono tabular-nums">
              {numBullets} {numBullets === 1 ? "bala" : "balas"}
            </div>
            <span className="text-[10px] text-[var(--text-3)] block font-mono">Intentos garantizados</span>
          </div>

          <div className="space-y-1">
            <span className="text-[10px] text-[var(--text-2)] uppercase font-mono block">Prob. Aprobar ≥ 1</span>
            <div
              className={`text-2xl font-black font-mono tabular-nums ${
                passProb >= 0.9 ? "text-[var(--profit)]" : passProb >= 0.7 ? "text-[var(--text-2)]" : "text-[var(--text-2)]"
              }`}
            >
          ...
          <div className="space-y-1">
            <span className="text-[10px] text-[var(--text-2)] uppercase font-mono block">Esperanza Matemática (EV)</span>
            <div
              className={`text-2xl font-black font-mono tabular-nums ${
                expectedValue > 0 ? "text-[var(--profit)]" : "text-[var(--loss)]"
              }`}
            >
              {expectedValue > 0 ? "+" : ""}${expectedValue.toFixed(2)}
            </div>
            <span className="text-[10px] text-[var(--text-3)] block font-mono">Retorno neto esperado</span>
          </div>

          <div className="space-y-1">
            <span className="text-[10px] text-[var(--text-2)] uppercase font-mono block">Cosecha Regla 80/20</span>
            <div className="text-base font-bold text-[var(--text-1)] font-mono tabular-nums mt-1">
              ${(payoutTarget * 0.8).toFixed(0)} <span className="text-xs text-[var(--text-2)] font-normal">Banco</span> / ${(payoutTarget * 0.2).toFixed(0)} <span className="text-xs text-[var(--text-2)] font-normal">Caja</span>
            </div>
            <span className="text-[10px] text-[var(--text-3)] block font-mono">Preservación de capital</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-mono text-[var(--text-2)]">
            ...
            <span className="font-bold text-[var(--text-1)]">{(passProb * 100).toFixed(1)}%</span>
          </div>
          <div className="w-full h-3 bg-[var(--surface-1)] rounded-full overflow-hidden border border-[var(--border)]">
            <div
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-[var(--text-2)]" />
              <h2 className="text-xl font-black text-[var(--text-1)] tracking-tight">
          ...
          <div className="flex flex-wrap items-center gap-1.5 bg-[var(--surface-1)] border border-white/[0.08] p-1.5 rounded-xl">
            {["ALL", "DOCTRINA", "MATEMÁTICA", "OPERATIVA", "PSICOTRADING", "EJECUCIÓN"].map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-all ${
                  activeCategory === cat
                    ? "bg-[var(--surface-3)] text-[var(--text-1)] font-black shadow-md "
            ...
            return (
              ...
              >
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="px-2 py-0.5 rounded-md bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text-2)] text-xs font-mono font-bold">
                      {mod.number}
                    </span>
                  ...
                  <h3 className="text-sm font-bold text-[var(--text-1)] group-hover:text-[var(--text-1)] transition-colors leading-snug">
                    {mod.title}
                  </h3>
                ...
                <div className="space-y-3">
                  {/* Key Rule Box */}
                  <div className="bg-[var(--surface-1)] border-l-2 border-[var(--border)] p-2.5 rounded-r-lg text-xs text-[var(--text-1)] italic font-sans leading-tight">
                    &quot;{mod.keyRule}&quot;
                  </div>
