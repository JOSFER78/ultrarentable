...
import { AlertTriangle, Database, FlaskConical, Hash, RefreshCw, Search, ShieldCheck, Sparkles, Workflow, type LucideIcon } from "lucide-react";
...
// Mapeo estado-API → chip unificado en lib/status-map.ts (única fuente de verdad).
const STAGES = [
  ["01", "ORIGEN", "EXTRACTED_UNVERIFIED", "Hipótesis/estrategia recibida con procedencia y hash."],
...
export default function StrategiesPage() {
  ...
  const assets = useMemo(() => Array.from(new Set(items.map((x) => x.symbol).filter((s): s is string => Boolean(s)))).sort(), [items]);
  const statuses = useMemo(() => Array.from(new Set(items.map((x) => x.validation_status).filter((s): s is string => Boolean(s)))).sort((a,b) => statusRank(a) - statusRank(b)), [items]);
  const filtered = useMemo(() => {
    ...
    return items.filter((x) => {
      ...
      const ra = statusRank(a.validation_status); const rb = statusRank(b.validation_status);
  ...
  return <main className="min-h-screen bg-slate-950 px-3 py-5 text-slate-100 md:px-6"><div className="mx-auto max-w-[1600px] space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[1fr_420px]"><div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5"><div className="flex items-center gap-2"><FlaskConical className="h-5 w-5 text-amber-400"/><h2 className="text-xl font-bold">Mejora e investigación</h2></div><p className="mt-1 text-sm text-slate-400">Extrae hipótesis reales de SQX. Después se investigan, optimizan y validan en las fases correspondientes. Esta pantalla no decide rentabilidad por sí sola.</p><div className="mt-4 flex flex-col gap-2 md:flex-row"><input value={project} onChange={(e) => setProject(e.target.value)} placeholder="Proyecto SQX real" className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm"/><button onClick={() => void extract()} disabled={extracting} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold disabled:opacity-50">{extracting ? "Extrayendo…" : "Extraer"}</button></div><div className="mt-4 text-xs text-slate-500">SQX: <span className="text-slate-300">{sqx?.status ?? "NO EVIDENCE"}</span></div></div>
      <aside className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">{selected ? <><div className="flex items-start justify-between gap-3"><div><div className="text-xs uppercase tracking-wide text-slate-500">Ficha canónica</div><h2 className="mt-1 text-xl font-bold">{selected.name}</h2></div><span className={`rounded-full border px-2 py-1 text-[10px] font-bold ${tone(selected.validation_status)}`}>{statusLabel(selected.validation_status)}</span></div><div className="mt-5 space-y-2">{[["Activo",selected.symbol],["Timeframe",selected.timeframe],["Origen",selected.source_project ?? selected.source_engine],["Dataset",selected.dataset_id],["Dataset hash",selected.dataset_hash],["Strategy hash",selected.strategy_hash],["Artifact hash",selected.source_artifact_sha256],["Creada",selected.created_at]].map(([k,v]) => <div key={k} className="rounded-lg border border-slate-800 bg-slate-950/50 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-600">{k}</div><div className="mt-1 break-all font-mono text-xs text-slate-300">{k === "Dataset hash" || k === "Strategy hash" || k === "Artifact hash" ? <EvidenceLink strategyHash={selected.strategy_hash} datasetHash={selected.dataset_hash} engineVersion={null} commitSha={null}><span>{v || "NO EVIDENCE"}</span></EvidenceLink> : (v || "NO EVIDENCE")}</div></div>)}</div><div className="mt-4 grid grid-cols-2 gap-2"><Link href="/estrategias/2-explorador-excel" className="rounded-lg border border-slate-700 px-3 py-2 text-center text-xs font-semibold">Candidatos</Link><Link href="/estrategias/4-panel-investigador" className="rounded-lg border border-slate-700 px-3 py-2 text-center text-xs font-semibold">Investigar</Link></div></> : <div className="py-12 text-center text-slate-500">Selecciona una estrategia.</div>}</aside></section>
