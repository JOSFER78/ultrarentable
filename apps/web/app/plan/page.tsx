  ...
  return (
    <div className="w-full max-w-[1100px] mx-auto space-y-6 font-sans pb-16">
      <div className="bg-[var(--surface-1)] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 md:p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black text-[var(--text-1)] tracking-tight">Plan del proyecto</h1>
              <p className="text-[13px] text-[var(--text-3)] mt-0.5">
                Fases F00–F09 leídas en vivo de <code className="font-mono text-[var(--text-2)]">orchestration/state/plan/bloques/</code>,
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/[0.08] bg-white/[0.03] text-[12px] text-[var(--text-1)] hover:border-white/20 hover:text-[var(--text-1)] transition-colors disabled:opacity-50"
          >
      ...
      {state.status === "loading" && (
        <div className="bg-[var(--surface-1)] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-10 text-center">
          <RefreshCw className="w-5 h-5 mx-auto mb-3 text-[var(--text-3)] animate-spin" />
          <p className="text-sm text-[var(--text-3)] font-mono">Leyendo el plan…</p>
        </div>
      )}

      {state.status === "error" && (
        <div className="bg-[var(--surface-1)] backdrop-blur-xl border border-[var(--loss)] rounded-2xl p-8 text-center">
          <AlertCircle className="w-6 h-6 mx-auto mb-3 text-[var(--loss)]" />
          <h2 className="text-sm font-bold text-[var(--text-1)] font-mono uppercase tracking-wide">Sin datos del plan</h2>
          <p className="mt-2 text-[13px] text-[var(--text-3)] max-w-md mx-auto">
      ...
      {state.status === "ready" && bloques.length === 0 && (
        <div className="bg-[var(--surface-1)] backdrop-blur-xl border border-[var(--border)] rounded-2xl p-8 text-center">
          <FileWarning className="w-6 h-6 mx-auto mb-3 text-[var(--text-2)]" />
          <h2 className="text-sm font-bold text-[var(--text-1)] font-mono uppercase tracking-wide">Sin datos del plan</h2>
          <p className="mt-2 text-[13px] text-[var(--text-3)] max-w-md mx-auto">
      ...
      {state.status === "ready" && errores.length > 0 && (
        <div className="bg-[var(--surface-2)] border border-[var(--border)] rounded-xl p-4">
          <div className="flex items-center gap-2 text-[var(--text-2)] text-[12px] font-mono uppercase tracking-wide">
            <AlertCircle className="w-4 h-4" />
            {errores.length} fichero(s) de plan con frontmatter no legible (excluidos del grafo)
          </div>
          <ul className="mt-2 space-y-1">
            {errores.map((e) => (
              <li key={e.archivo} className="text-[12px] text-[var(--text-2)] font-mono">
                {e.archivo}: <span className="text-[var(--text-3)]">{e.error}</span>
              </li>
            ))}
          </ul>
        </div>
