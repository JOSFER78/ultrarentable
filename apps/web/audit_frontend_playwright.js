...
async function runAudit() {
  ...
  for (const p of PAGES_TO_AUDIT) {
    ...
    try {
      ...
      console.log(`  ✅ HTTP ${status} | Heading: "${heading ? heading.trim().substring(0, 40) : 'N/A'}..." | Screenshot guardado (${p.name}.png)`);
    } catch (err) {
  ...
  console.log('\n📊 Auditoría Finalizada con éxito. Resumen guardado en ' + reportFile);
}
