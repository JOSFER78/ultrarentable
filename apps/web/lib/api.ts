...
export async function extractStrategyLabProject(projectName: string): Promise<StrategyLabExtractionResult> { if (!projectName.trim()) throw new Error("PROJECT_NAME_REQUIRED"); return fetchJson<StrategyLabExtractionResult>(`/api/v2/strategy-lab/extract/${encodeURIComponent(projectName)}`, { method: "POST" }); }
...
export const api = {
  ...
  getCandidates, getCertifiedStrategies, getCertifiedMetaStrategies, getDiscoveryStatus, getStrategyLabOverview, getStrategyLabStrategies, getStrategyLabSQXStatus, extractStrategyLabProject, getLineageTree, verifyCertificate, runPolicyImpactAnalysis, triggerResearchDebate, synthesizeStrategyMutation, enqueueDurableJob, getDurableJobs, evaluateForwardSufficiency, executeBacktest,
};
