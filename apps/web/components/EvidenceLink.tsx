...
export default function EvidenceLink({ strategyHash, datasetHash, engineVersion, commitSha, children }: EvidenceLinkProps) {
  ...
  return (
    <span className="group relative inline-flex items-center gap-1">
      ...
      <span className="pointer-events-none absolute bottom-full left-0 z-30 mb-2 hidden w-72 rounded-lg border border-slate-700 bg-slate-950 p-3 shadow-2xl group-hover:block">
          ...
          return (
            <span key={label} className="mb-1 block font-mono text-[10px] leading-snug">
              ...
              {has ? (
                ...
                <span className="font-bold text-amber-400">NO EVIDENCE</span>
              )}
            </span>
          );
        })}
        <span className="mt-1 block text-[9px] leading-snug text-slate-600">
