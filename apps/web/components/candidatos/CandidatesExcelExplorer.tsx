    ...
    <div className="w-full max-w-[1720px] mx-auto space-y-5 font-sans">
      ...
      <div className="bg-[#090d16]/90 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 md:p-6 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 flex-wrap font-mono">
          <button
            onClick={exportToCSV}
            className="inline-flex items-center px-3.5 py-1.5 rounded-xl text-xs font-bold bg-emerald-950/80 hover:bg-emerald-900 text-emerald-300 border border-emerald-700/60 shadow-sm transition active:scale-95 cursor-pointer"
          >
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center px-3.5 py-1.5 rounded-xl text-xs font-bold bg-[#050811] hover:bg-slate-800 text-slate-200 border border-white/[0.1] transition active:scale-95 shadow-sm cursor-pointer"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-[#090d16]/90 backdrop-blur-xl p-2.5 rounded-2xl border border-white/[0.08] shadow-xl">
        ...
        <div className="flex items-center gap-1 self-end sm:self-auto bg-[#050811] p-1 rounded-xl border border-white/[0.08] text-[10px] font-mono">
          ...
          {(["compact", "normal", "spacious"] as DensityType[]).map((d) => (
            <button
              ...
              className={`px-2 py-1 rounded-lg capitalize transition font-bold cursor-pointer ${
                density === d ? "bg-indigo-600 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
              }`}
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 font-mono">
        <div className="p-3.5 bg-[#090d16]/90 backdrop-blur-md rounded-xl border border-white/[0.08]">
          <span className="text-[10px] uppercase text-slate-400 font-bold block">Estrategias Listadas</span>
          <span className="text-lg font-black text-white tabular-nums">{kpis.total}</span>
        </div>
        <div className="p-3.5 bg-[#090d16]/90 backdrop-blur-md rounded-xl border border-white/[0.08]">
          <span className="text-[10px] uppercase text-slate-400 font-bold block">Certificadas Reales</span>
          <span className="text-lg font-black text-emerald-400 tabular-nums">{kpis.approvedCount}</span>
        </div>
        <div className="p-3.5 bg-[#090d16]/90 backdrop-blur-md rounded-xl border border-white/[0.08]">
        ...
        <div className="p-3.5 bg-[#090d16]/90 backdrop-blur-md rounded-xl border border-white/[0.08]">
          ...
          <span
            className={`text-lg font-black tabular-nums ${
              kpis.total === 0 ? "text-slate-500" : kpis.passRate > 0 ? "text-emerald-400" : "text-slate-400"
            }`}
          >
        ...
        <div className="p-3.5 bg-[#090d16]/90 backdrop-blur-md rounded-xl border border-white/[0.08]">
          ...
          <span
            className={`text-lg font-black tabular-nums ${
              kpis.avgDd === null ? "text-slate-500" : kpis.avgDd <= 4.5 ? "text-emerald-400" : "text-amber-400"
            }`}
          >
      ...
      {/* BUSCADOR Y FILTROS */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-[#090d16]/90 backdrop-blur-md p-3.5 rounded-2xl border border-white/[0.08] font-mono text-xs shadow-lg">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-[#050811] rounded-xl border border-white/[0.1] text-slate-100 placeholder-slate-500 text-xs focus:border-indigo-500 focus:outline-none"
          />
        </div>
        ...
        <div>
          <select
            ...
            className="w-full py-2 px-3 bg-[#050811] rounded-xl border border-white/[0.1] text-slate-200 text-xs focus:border-indigo-500 focus:outline-none"
          >
      ...
      <div className="bg-[#090d16]/90 backdrop-blur-xl rounded-2xl border border-white/[0.08] overflow-hidden shadow-2xl">
        <div className="overflow-x-auto max-h-[750px]">
          <table className="w-full text-left border-collapse font-mono text-xs">
            <thead className="bg-[#050811] sticky top-0 z-10 border-b border-white/[0.08] text-slate-400 text-[11px] select-none">
              <tr>
            ...
            <tbody className="divide-y divide-white/[0.05]">
              {loading ? (
                <tr>
                  <td colSpan={18} className="py-16 text-center text-slate-500 text-xs font-mono">
                  ...
                  return (
                    <tr key={row.candidate_id || idx} className="hover:bg-white/[0.03] transition">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                              isCertified
                                ...
                                : "bg-slate-800/80 text-slate-300 border-white/[0.08]"
                            }`}
                          >
                            {row.status}
                          </span>
                      ...
                      <td className={`${pad} text-right font-mono tabular-nums`}>
                        <span className="px-1.5 py-0.5 rounded border border-white/[0.08] text-[11px] text-slate-300">
                      ...
                      <td className={`${pad} text-right text-slate-300 tabular-nums`}>
                      ...
                      <td className={`${pad} text-right text-slate-300 tabular-nums`}>
                        {row.trades_oos === undefined ? <span className="text-slate-500">NO EVIDENCE</span> : row.trades_oos}
                      </td>
                      <td className={`${pad} text-right text-indigo-300 tabular-nums`}>
                        {row.sharpe_ratio === undefined ? <span className="text-slate-500">NO EVIDENCE</span> : row.sharpe_ratio.toFixed(2)}
                      </td>
                      <td className={`${pad} text-right text-slate-400 tabular-nums`}>
                      ...
                      <td className={`${pad} text-right text-purple-300 tabular-nums`}>
                      ...
                      <td className={`${pad} text-center`}>
                        {row.strategy_sha256 ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-[#050811] border border-white/[0.08] hover:border-white/[0.16] text-[10px] text-slate-400 cursor-pointer"
                      ...
                      <td className={`${pad} text-center`}>
                        <Link
                          href="/estrategias"
                          className="inline-flex items-center px-2 py-0.5 rounded bg-indigo-950/80 hover:bg-indigo-900 text-indigo-300 border border-indigo-700/70 text-[10px] font-bold transition shadow-sm"
                        >
                          Probar
                        </Link>
                      </td>
