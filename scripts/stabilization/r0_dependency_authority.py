...
This check prevents silent drift, duplicate dependency authorities, and lockfile
mismatches from becoming runtime-only failures.
"""
...
def check_npm(failures: list[str]) -> None:
    ...
    if not isinstance(root_lock, dict):
        ...
        if root_lock.get("name") != package.get("name"):
            fail("root package name differs from package-lock.json", failures)
        if root_lock.get("version") != package.get("version"):
            fail("root package version differs from package-lock.json", failures)
        if root_lock.get("workspaces") != package.get("workspaces"):
            fail("root workspaces differ from package-lock.json", failures)
    ...
    present = [str(path.relative_to(ROOT)) for path in duplicate_manifests if path.is_file()]
    if present:
...
def check_python(failures: list[str]) -> None:
    ...
    root_lock = next((pkg for pkg in root_packages if pkg.get("name") == project.get("name")), None)
    ...
    present = [str(path.relative_to(ROOT)) for path in competing if path.is_file()]
    if present:
...
def main() -> int:
    ...
    result = {
        "check": "R0.1_DEPENDENCY_AUTHORITY",
