"""scripts/mine_and_certify_all_intraday.py
Minería y Certificación 11/11 Formal de Estrategias ULTRA Intradía con Piramidación.
Cubre marcos temporales: 1m, 5m, 15m, 1h para ETH, BTC, SOL, SUI, XRP, BNB, AVAX, LINK, DOGE.
...
def mine_and_certify():
    print("==========================================================================", flush=True)
    print("⚡ INICIANDO MINERÍA Y CERTIFICACIÓN 11/11 ULTRA INTRADÍA (1m, 5m, 15m, 1h)", flush=True)
    print("==========================================================================", flush=True)
    ...
    for m_file in manifest_files:
        try:
            ...
            print(f"⚠️ Error leyendo manifiesto {m_file}: {e}", flush=True)

    print(f"📊 Datasets intradía encontrados: {len(intraday_datasets)}", flush=True)
    ...
    # Búsqueda multi-arquetipo optimizada para asimetría taleb y alta relacion beneficio/riesgo
    search_grid = [
        ...
        ("TREND_FOLLOWING", 8, 24, 14, 50.0, 50.0, 1.5, 5.0, 0.015, 2, None, 0),
    ...
    for ds_idx, (data_file, m_file, symbol, timeframe, mdata) in enumerate(intraday_datasets, 1):
        try:
            ...
            if size_mb > 25.0:
                print(f"[{ds_idx}/{len(intraday_datasets)}] Omitiendo dataset masivo (>25MB): {symbol} {timeframe} ({size_mb:.1f} MB)", flush=True)
                continue
            ...
            print(f"[{ds_idx}/{len(intraday_datasets)}] Evaluando dataset {symbol} {timeframe} ({total_bars} velas total, {len(candles_blind_oos)} OOS)", flush=True)
            ...
            for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk) in enumerate(search_grid, 1):
                ...
                if (oos_bt.profit_factor >= min_pf_limit 
                    ...
                    print(f"   🎯 Candidato Prometedor {strat_id}: PF_OOS={oos_bt.profit_factor:.2f}, DD_OOS={oos_bt.max_drawdown_pct:.2f}%, Trades_OOS={oos_bt.total_trades}, PnL=${oos_bt.net_profit_usd:.2f}", flush=True)
                    ...
                    print(f"   🛡️ Evaluación 11 Gates {strat_id}: Passed={passed_count}/11, Verdict={verdict.certified_status}, Score={overall_score:.1f}", flush=True)
                    ...
                    if verdict.is_certified or passed_count == 11:
                        ...
                        conn.commit()
                        certified_strategies.append((snapshot.strategy_id, symbol, timeframe, oos_bt.profit_factor, oos_bt.max_drawdown_pct, oos_bt.total_trades))
                        print(f"   ✅ ¡ESTRATEGIA CERTIFICADA REGISTRADA! -> {snapshot.strategy_id} (DD: {oos_bt.max_drawdown_pct:.2f}%, PF: {oos_bt.profit_factor:.2f}, TF: {timeframe})", flush=True)

        except Exception as e:
            print(f"❌ Error procesando {data_file} ({symbol}): {e}", flush=True)

    conn.close()
    print("==========================================================================", flush=True)
    print(f"🏁 MINERÍA FINALIZADA. TOTAL ESTRATEGIAS INTRADÍA CERTIFICADAS 11/11: {len(certified_strategies)}", flush=True)
    for s_info in certified_strategies:
        print(f"   🏆 {s_info}", flush=True)
    print("==========================================================================", flush=True)
