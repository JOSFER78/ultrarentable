"""scripts/mine_and_certify_crypto_master.py
Motor de Minería Cuantitativa y Certificación 11/11 ULTRA Intradía para Criptoactivos con Piramidación Activa.
...
def save_candidate_to_sqlite(snapshot, symbol, timeframe, fname, file_sha256, initial_cap, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    try:
        ...
        print(f"❌ Error guardando en SQLite {snapshot.strategy_id}: {e}", flush=True)
        return False


def get_ultra_crypto_configs():
    """Genera configuraciones cuantitativas refinadas para asimetría y piramidación 2-3 tiers."""
    return [
        ...
        ("MOMENTUM_BREAKOUT", 8, 21, 14, 52.0, 48.0, 1.5, 4.8, 0.02, 2, "RR_DYNAMIC", "ATR_REGIME", 15),
...
def run_mining_and_certification():
    print("==========================================================================", flush=True)
    print("⚡ MOTOR DE MINERÍA Y CERTIFICACIÓN 11/11 ULTRA CRYPTO INTRADÍA", flush=True)
    print("==========================================================================", flush=True)
    ...
    print(f"📊 Datasets Cripto objetivo identificados: {len(datasets)}", flush=True)
    ...
    search_configs = get_ultra_crypto_configs()
    ...
    for (target_sym, target_tf), (data_file, m_file, symbol, timeframe, mdata) in sorted(datasets.items()):
        try:
            ...
            if size_mb > 35.0:
                print(f"⏩ Omitiendo dataset masivo {symbol} {timeframe} ({size_mb:.1f} MB)", flush=True)
                continue
            ...
            print(f"\n🔍 Evaluando {symbol} {timeframe} ({total_bars} barras total, {len(candles_blind_oos)} OOS)...", flush=True)
            ...
            for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, exit_fam, vol_filt, brk_lk) in enumerate(search_configs, 1):
                ...
                if (oos_bt.profit_factor >= min_pf_limit 
                    ...
                    print(f"   🛡️ {strat_id} ({arch}): PF={oos_bt.profit_factor:.2f}, DD={oos_bt.max_drawdown_pct:.2f}%, Trades={oos_bt.total_trades} -> Gates: {passed_count}/11 (Verdict: {verdict.certified_status})", flush=True)
                    ...
                    if verdict.is_certified or passed_count == 11:
                        ...
                        print(f"   🏆 ¡ESTRATEGIA CERTIFICADA 11/11! -> {snapshot.strategy_id} ({symbol} {timeframe}) | PF={oos_bt.profit_factor:.2f} | DD={oos_bt.max_drawdown_pct:.2f}% | ROI/mes={monthly_roi_pct:.1f}%", flush=True)

        except Exception as e:
            print(f"Error procesando {symbol} {timeframe}: {e}", flush=True)

    print("\n==========================================================================", flush=True)
    print(f"🏁 MINERÍA FINALIZADA. TOTAL ESTRATEGIAS CERTIFICADAS ULTRA CRYPTO 11/11: {len(certified_strategies)}", flush=True)
    print("==========================================================================", flush=True)
