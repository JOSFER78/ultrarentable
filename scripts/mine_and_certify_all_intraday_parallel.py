"""scripts/mine_and_certify_all_intraday_parallel.py
High-Performance Parallel Quantitative Mining & 11/11 Gate Certification Engine for ULTRA Intraday Strategies with Pyramiding.
Covers 1m, 5m, 15m, 1h timeframes across Crypto (ETH, BTC, SOL, SUI, XRP, BNB, AVAX, LINK, DOGE), Futures, and Forex.
...
def evaluate_dataset(args):
    data_file, m_file, symbol, timeframe, mdata = args
    certified_results = []
    ...
    # Broad quantitative search grid for intraday high payoff & Taleb convexity with active pyramiding
    search_grid = [
        ...
        ("TREND_FOLLOWING", 10, 30, 14, 52.0, 48.0, 1.8, 6.0, 0.015, 2, "ATR_REGIME", 0),
        ...
        ("MOMENTUM_BREAKOUT", 5, 15, 14, 50.0, 50.0, 1.2, 4.0, 0.02, 2, None, 10),
...
def save_batch_to_db(items):
    ...
    for item in items:
        ...
        is_bt = item["is_bt"]
        oos_bt = item["oos_bt"]
        scorecard_payload = item["scorecard_payload"]
        certified_at_iso = item["certified_at_iso"]
...
def main():
    print("==========================================================================", flush=True)
    print("⚡ MINERÍA Y CERTIFICACIÓN PARALELA 11/11 ULTRA INTRADÍA", flush=True)
    print("==========================================================================", flush=True)
    ...
    print(f"📊 Datasets intradía identificados: {len(intraday_datasets)}", flush=True)

    total_certified = 0

    with ProcessPoolExecutor(max_workers=6) as executor:
        futures = {executor.submit(evaluate_dataset, ds): ds for ds in intraday_datasets}
        for future in as_completed(futures):
            ...
            try:
                ...
                if res_list:
                    save_batch_to_db(res_list)
                    total_certified += len(res_list)
                    for item in res_list:
                        snap = item["snapshot"]
                        bt = item["oos_bt"]
                        print(f"  🏆 CERTIFICADA 11/11: {snap.strategy_id} | {sym} {tf} | PF={bt.profit_factor:.2f} | DD={bt.max_drawdown_pct:.2f}% | Trades={bt.total_trades} | PnL=${bt.net_profit_usd:.2f}", flush=True)
            except Exception as e:
                print(f"❌ Error procesando {sym} ({tf}): {e}", flush=True)

    print("==========================================================================", flush=True)
    print(f"🏁 MINERÍA FINALIZADA. TOTAL ESTRATEGIAS INTRADÍA CERTIFICADAS 11/11 REGISTRADAS: {total_certified}", flush=True)
    print("==========================================================================", flush=True)
