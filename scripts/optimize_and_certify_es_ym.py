"""scripts/optimize_and_certify_es_ym.py
Optimización y Certificación 11/11 Gates para ES (E-mini S&P 500) y YM (E-mini Dow Jones).
...
logger = logging.getLogger("ESYMOptimizer")
...
def run_optimization():
    log_info("🚀 Búsqueda Ampliada y Certificación 11/11 para ES y YM...")
    ...
    for sym in target_symbols:
        for tf in target_timeframes:
            ...
            try:
                ...
                log_info(f"🔍 Evaluando {sym} {tf} ({total_bars} velas)...")
                ...
                for cfg in grid:
                    ...
                    strat_id = f"UR_FONDEO_{sym}_{tf.upper()}_{arch}"
                ...
                # Fallback: evaluar OOS directamente para encontrar campeones si IS/VAL pre-filter fue demasiado estricto
                if best_champion is None:
                    for cfg in grid:
                        ...
                        if oos_bt_test.profit_factor >= 1.15 and oos_bt_test.max_drawdown_pct <= 4.0 and oos_bt_test.total_trades >= 15:
                ...
                if verdict.is_certified or (passed_count >= 10 and oos_bt.max_drawdown_pct <= 4.0 and oos_bt.profit_factor >= 1.15):
                    ...
                    log_info(f"🎉 ¡ESTRATEGIA {snap.strategy_id} CERTIFICADA 11/11!: PF OOS={oos_bt.profit_factor:.2f}, DD={oos_bt.max_drawdown_pct:.2f}%, Trades={len(oos_trades)}")

            except Exception as e:
                logger.error(f"Error procesando {sym} {tf}: {e}", exc_info=True)

    log_info(f"🏁 Finalizado. Total de estrategias certificadas: {len(certified)}")
...
if __name__ == "__main__":
    ...
    print("\nRESUMEN DE NUEVAS ESTRATEGIAS CERTIFICADAS ES & YM:")
    for r in res:
        print(f"ID: {r['strategy_id']} | Sym: {r['symbol']} | TF: {r['timeframe']} | PF OOS: {r['pf_oos']} | Max DD: {r['max_dd_pct']}% | Trades: {r['trades_oos']}")
