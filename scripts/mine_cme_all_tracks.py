"""scripts/mine_cme_all_tracks.py
Motor Unificado de Minería Cuantitativa y Certificación 11/11 Gates para CME Futures (NQ, ES, YM, GC, SI).
Cubre TRACK_FONDEO y TRACK_ULTRA en temporalidades intradía (5m, 15m, 1h, 4h).
...
logger = logging.getLogger("CMEMultiTrackMiner")
...
def build_cme_snapshot(
    ...
    pyr_policy = PyramidingPolicy(
        enabled=pyramiding_enabled,
        max_tiers=3 if pyramiding_enabled else 1,
        tiers=[
            PyramidingTier(trigger_pnl_atr_mult=1.5, added_size_mult=0.5, trail_stop_to_breakeven=True),
            PyramidingTier(trigger_pnl_atr_mult=3.0, added_size_mult=0.5, trail_stop_to_breakeven=True),
        ] if pyramiding_enabled else [],
    )
    ...
    return StrategySnapshot.create_and_hash(
        ...
        pyramiding_policy=pyr_policy,
        session_window=session_win,
    )


def run_cme_multi_track_mining():
    log_info("🚀 Minería Cuantitativa Multi-Ruta (FONDEO & ULTRA) en CME Futures (NQ, ES, YM, GC, SI)...")

    bt_engine = EventBacktestEngine()
    gates_orch = GatePipelineOrchestrator()
    cert_reg = CertificationRegistry()

    symbols = ["NQ", "ES", "YM", "GC", "SI"]
    timeframes = ["5m", "15m", "1h", "4h"]
    routes = [StrategyRoute.FONDEO, StrategyRoute.ULTRA]
        ...
        for tf in timeframes:
            ...
            try:
                ...
                for route in routes:
                    ...
                    min_trades_limit = 10 if is_ultra else 15
                    ...
                    for cfg in grid:
                        ...
                        strat_id = f"UR_{route.value}_{sym}_{tf.upper()}_{arch}"
                        ...
                        snap = build_cme_snapshot(
                            ...
                            sl_atr_mult=sl_m,
                            tp_atr_mult=tp_m,
                            risk_pct=risk_p if not is_ultra else 0.02,
                            donchian_lookback=donch_l,
                            pyramiding_enabled=is_ultra,
                        )

                        is_bt = bt_engine.run_backtest(snap, candles_is, initial_capital_usd=initial_cap)
                        if is_bt.profit_factor < 1.02 or is_bt.max_drawdown_pct > (max_dd_limit * 1.5) or is_bt.total_trades < 3:
                            continue

                        val_bt = bt_engine.run_backtest(snap, candles_val, initial_capital_usd=initial_cap)
                        if val_bt.profit_factor < 1.02 or val_bt.max_drawdown_pct > (max_dd_limit * 1.5) or val_bt.total_trades < 2:
                            continue

                        score = (val_bt.profit_factor * 20.0) - (val_bt.max_drawdown_pct * 5.0) + (val_bt.total_trades * 0.5)
                        if score > best_score:
                            best_score = score
                    ...
                    if (oos_bt.profit_factor < min_pf_limit or 
                        oos_bt.max_drawdown_pct > max_dd_limit or 
                        oos_bt.total_trades < min_trades_limit or 
                    ...
                    if verdict.is_certified or (passed_count >= 10 and oos_bt.max_drawdown_pct <= max_dd_limit and oos_bt.profit_factor >= min_pf_limit):
                        # Evidencia física
                        ...
                        scorecard_payload = {
                            "source": f"Autonomous Real-Only CME Discovery ({route.value} Track)",
                            "strategy_snapshot_hash": snap.canonical_hash,
                            "dataset_sha256": real_file_sha256,
                            "route": route.value,
                            "initial_capital_usd": initial_cap,
                            ...
                            "oos_returns": [t["net_pnl_usd"] for t in trades_raw],
                            "certification_status": f"{route.value}_CERTIFIED",
                        ...
                        log_info(f"🎉 ¡ESTRATEGIA {snap.strategy_id} ({route.value}) CERTIFICADA 11/11!: PF OOS={oos_bt.profit_factor:.2f}, DD={oos_bt.max_drawdown_pct:.2f}%, Trades={len(oos_trades)}, Mensual=+{monthly_roi_pct:.2f}%")

            except Exception as e:
                logger.error(f"Error procesando {sym} {tf}: {e}", exc_info=True)

    log_info(f"🏁 Minería Multi-Ruta CME completada: {len(all_certified)} estrategias certificadas 11/11.")

    try:
        meta_res = ensure_meta_strategies(["FONDEO", "ULTRA"])
        log_info(f"Meta-Estrategias actualizadas: {meta_res}")
    except Exception as e:
...
if __name__ == "__main__":
    res = run_cme_multi_track_mining()
    print("\n" + "=" * 80)
    print("RESUMEN GENERAL DE ESTRATEGIAS CME CERTIFICADAS 11/11:")
    print("=" * 80)
    for r in res:
        print(f"[{r['route']}] ID: {r['strategy_id']} | Sym: {r['symbol']} | TF: {r['timeframe']} | Arch: {r['archetype']}")
