"""scripts/register_and_assemble_all_champions.py
Sellado de evidencia física y ensamblado canónico de Meta-FONDEO y Meta-ULTRA.
...
def certify_and_assemble_all():
    bt = EventBacktestEngine()
    gates_orch = GatePipelineOrchestrator()
    cert_reg = CertificationRegistry()

    champions_fondeo = [
        # (fname, sym, tf, lookback, ema_f, ema_s, sl_m, tp_m, risk_p)
        ("ds_trad_cl_1h_1711425600000_1787090400000.json", "CL", "1h", 15, 10, 30, 2.0, 6.0, 0.08),
        ("ds_trad_si_1h_1711425600000_1787090400000.json", "SI", "1h", 15, 20, 60, 2.0, 6.0, 0.08),
        ("ds_trad_nq_1h_1711425600000_1787090400000.json", "NQ", "1h", 10, 8, 24, 1.2, 4.5, 0.08),
        ("ds_trad_gc_1h_1711425600000_1787090400000.json", "GC", "1h", 30, 20, 60, 2.0, 4.5, 0.08),
    ]

    for fname, sym, tf, lookback, ema_f, ema_s, sl_m, tp_m, risk_p in champions_fondeo:
        ...
        snap = StrategySnapshot.create_and_hash(
            ...
            symbol=sym,
            timeframe=tf,
            archetype="DONCHIAN_MOMENTUM_BREAKOUT",
            entry_rules=entry,
            exit_rules=exit_m,
        ...
        conn.commit()
        conn.close()
        logger.info(f"✅ FONDEO Registrada y Sellada: {snap.strategy_id} (PF: {oos_res.profit_factor:.2f}, DD: {oos_res.max_drawdown_pct:.2f}%, Trades: {len(oos_trades)})")

    logger.info("⚙️ Ensamblando Meta-Estrategias duales (FONDEO & ULTRA)...")
    res_meta = ensure_meta_strategies(["ULTRA", "FONDEO"])
    logger.info(f"Resultado Meta-Estrategias: {res_meta}")


if __name__ == "__main__":
    certify_and_assemble_all()
