"""scripts/debug_single_dataset_gates.py
Ejecuta la evaluación de los 11 Gates sobre un dataset específico y muestra los resultados de cada gate.
"""
...
def run_debug(symbol_query="SUI", timeframe_query="4h"):
    ...
    for m_file in manifest_files:
        try:
            with open(m_file) as f:
                d = json.load(f)
            sym = d.get("symbol", "").upper().replace("-", "").replace("_", "")
            tf = d.get("interval", "").lower()
            if symbol_query.upper() in sym and tf == timeframe_query.lower():
                df = m_file.replace("_manifest.json", ".json")
                if os.path.exists(df):
                    target_data_file = df
                    target_manifest = m_file
                    mdata = d
                    break
        except Exception:
            pass
    ...
    if not target_data_file:
        print(f"Dataset not found for {symbol_query} {timeframe_query}")
        return

    print(f"Testing Dataset: {target_data_file}")
    ...
    print(f"Total bars: {total_bars}, IS: {len(candles_is)}, PreOOS: {len(candles_pre_oos)}, Blind OOS: {len(candles_blind_oos)}")

    backtest_engine = EventBacktestEngine()
    gates_orchestrator = GatePipelineOrchestrator()
    ultra_discovery = UltraDiscoveryEngine()

    file_sha256 = mdata.get("checksum_sha256", "hash")
    dataset_id = Path(target_data_file).stem

    configs = [
        ("TREND_FOLLOWING", 5, 20, 14, 52.0, 48.0, 1.2, 4.5, 0.02, 2, "RR_DYNAMIC", "ATR_REGIME", 0),
        ...
        ("MOMENTUM_BREAKOUT", 10, 30, 14, 50.0, 50.0, 1.5, 5.5, 0.015, 2, "RR_DYNAMIC", None, 20),
        ("RSI_MOMENTUM", 8, 24, 14, 55.0, 45.0, 1.5, 5.0, 0.015, 2, "RR_DYNAMIC", "ATR_REGIME", 0),
    ]

    for idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, exit_fam, vol_filt, brk_lk) in enumerate(configs, 1):
        strat_id = f"UR_ULTRA_{symbol_query}_{timeframe_query}_dbg{idx}"

        snapshot = ultra_discovery.generate_candidate_blueprint(
            strategy_id=strat_id,
            symbol=symbol_query,
            timeframe=timeframe_query,
        ...
        print(f"\nConfig {idx} ({arch}): PF_OOS={oos_bt.profit_factor:.2f}, DD_OOS={oos_bt.max_drawdown_pct:.2f}%, Trades_OOS={oos_bt.total_trades}, PnL=${oos_bt.net_profit_usd:.2f}")
        ...
        candidate_info = {
            ...
            "symbol": symbol_query, "timeframe": timeframe_query, "profit_factor_oos": oos_bt.profit_factor,
            "max_drawdown_pct": oos_bt.max_drawdown_pct, "dataset_id": dataset_id,
            "dataset_filepath": str(target_data_file), "dataset_sha256": file_sha256, "trials_tested": 20,
        ...
        print(f"   -> Gates Passed: {passed_count}/11 (Overall Score: {gates_eval.get('overall_score', 0):.1f})")
        for g in gates_eval.get("gates", []):
            mark = "✅" if g.get("passed") else "❌"
            print(f"      {mark} Gate {g.get('gate_id'):02d} ({g.get('name')}): {g.get('verdict')}")

if __name__ == "__main__":
    sym = sys.argv[1] if len(sys.argv) > 1 else "SUI"
