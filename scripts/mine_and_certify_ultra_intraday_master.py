"""scripts/mine_and_certify_ultra_intraday_master.py
Master Quantitative Mining & 11/11 Gate Certification Engine for ULTRA Intraday Strategies with Pyramiding.
Timeframes: 1m, 5m, 15m, 1h.
Symbols: ETH, BTC, SOL, SUI, XRP, BNB, AVAX, LINK, DOGE, NQ, ES, YM, RTY, GC, SI, CL, EURUSD, GBPUSD, USDJPY, USDCAD, USDCHF, AUDUSD.
...
LOG_FILE = EVIDENCE_DIR / "ultra_master_mining.log"
...
def save_candidate_to_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute(
        ...
        log_msg(f"❌ Error guardando candidato en DB: {e}")
        return False


def process_dataset(dataset_tuple):
    ...
    # Highly robust quantitative search grid for intraday high payoff & Taleb convexity with active pyramiding
    search_grid = [
        # (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_m, tp_m, risk_p, py_tiers, vol_filt, brk_lk)
        ...
        ("TREND_FOLLOWING", 15, 45, 14, 53.0, 47.0, 2.0, 7.5, 0.015, 3, "ATR_REGIME", 0),
        ("TREND_FOLLOWING", 5, 20, 14, 50.0, 50.0, 1.2, 4.5, 0.02, 2, None, 0),
        ("MOMENTUM_BREAKOUT", 8, 24, 14, 50.0, 50.0, 1.5, 5.5, 0.02, 2, None, 15),
        ("MOMENTUM_BREAKOUT", 10, 30, 14, 52.0, 48.0, 1.8, 7.0, 0.015, 2, "ATR_REGIME", 20),
        ("MOMENTUM_BREAKOUT", 15, 40, 14, 53.0, 47.0, 2.0, 8.0, 0.015, 2, "ATR_REGIME", 25),
        ("RSI_MOMENTUM", 6, 18, 9, 50.0, 50.0, 1.2, 4.5, 0.02, 2, None, 0),
        ("RSI_MOMENTUM", 8, 24, 14, 55.0, 45.0, 1.5, 6.0, 0.015, 2, None, 0),
        ("RSI_MOMENTUM", 12, 36, 14, 56.0, 44.0, 1.8, 7.0, 0.015, 2, None, 0),
        ("EMA_CROSS", 5, 20, 14, 50.0, 50.0, 1.2, 4.5, 0.02, 2, None, 0),
        ("EMA_CROSS", 10, 30, 14, 52.0, 48.0, 1.5, 6.0, 0.015, 2, None, 0),
        ("EMA_CROSS", 15, 45, 14, 53.0, 47.0, 2.0, 7.5, 0.015, 3, None, 0),
        ("MEAN_REVERSION", 8, 21, 10, 65.0, 35.0, 1.5, 4.0, 0.02, 2, None, 0),
        ("MEAN_REVERSION", 10, 30, 14, 70.0, 30.0, 1.8, 4.5, 0.015, 2, None, 0),
    ]
    ...
    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk) in enumerate(search_grid, 1):
        ...
        if (oos_bt.profit_factor >= min_pf_limit 
            ...
            if verdict.is_certified or passed_count == 11:
                # Evidence physical file persistence
                ...
                save_candidate_to_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso)
...
def main():
    log_msg("==========================================================================")
    log_msg("⚡ MOTOR MASTER DE MINERÍA Y CERTIFICACIÓN 11/11 ULTRA INTRADÍA")
    log_msg("==========================================================================")
    ...
    log_msg(f"📊 Total datasets intradía identificados: {len(intraday_datasets)}")

    all_certified = []

    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process_dataset, ds): ds for ds in intraday_datasets}
            ...
            try:
                ...
                if res_list:
                    ...
                    for item in res_list:
                        snap = item["snapshot"]
                        bt = item["oos_bt"]
                        log_msg(f"🎯 CERTIFICADA 11/11: {snap.strategy_id} | {sym} {tf} | PF={bt.profit_factor:.2f} | DD={bt.max_drawdown_pct:.2f}% | Trades={bt.total_trades} | PnL=${bt.net_profit_usd:.2f}")
            except Exception as e:
                log_msg(f"❌ Error en worker dataset {sym} ({tf}): {e}")

    log_msg("==========================================================================")
    log_msg(f"🏁 PROCESO DE CERTIFICACIÓN COMPLETADO. TOTAL ESTRATEGIAS CERTIFICADAS ULTRA INTRADÍA: {len(all_certified)}")
    log_msg("==========================================================================")
