"""scripts/mine_and_certify_crypto_11gates.py
Minería Cuantitativa y Certificación 11/11 ULTRA Intradía para Criptoactivos con Piramidación Activa.
...
def save_candidate_to_sqlite(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute(
        ...
        print(f"❌ Error guardando en SQLite: {e}", flush=True)
        return False


def get_crypto_search_configs():
    """Matriz de búsqueda cuantitativa optimizada para asimetría y piramidación 2-3 tiers en Cripto."""
    return [
        # (archetype, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, exit_fam, vol_filt, brk_lk)
        # MOMENTUM_BREAKOUT
        ("MOMENTUM_BREAKOUT", 8, 21, 14, 52.0, 48.0, 1.2, 4.5, 0.02, 2, "RR_DYNAMIC", None, 15),
        ("MOMENTUM_BREAKOUT", 10, 30, 14, 50.0, 50.0, 1.5, 5.5, 0.015, 2, "RR_DYNAMIC", "ATR_REGIME", 20),
        ("MOMENTUM_BREAKOUT", 12, 36, 14, 53.0, 47.0, 1.5, 6.0, 0.02, 3, "RR_DYNAMIC", "ATR_REGIME", 25),
        ("MOMENTUM_BREAKOUT", 15, 45, 14, 55.0, 45.0, 1.8, 7.0, 0.015, 3, "RR_DYNAMIC", "ATR_REGIME", 20),
        ("MOMENTUM_BREAKOUT", 6, 18, 14, 50.0, 50.0, 1.2, 4.0, 0.02, 2, "RR_DYNAMIC", None, 10),
        ("MOMENTUM_BREAKOUT", 9, 26, 14, 52.0, 48.0, 1.4, 5.0, 0.02, 2, "RR_DYNAMIC", None, 15),

        # TREND_FOLLOWING
        ("TREND_FOLLOWING", 5, 20, 14, 52.0, 48.0, 1.2, 4.0, 0.02, 2, "RR_DYNAMIC", "ATR_REGIME", 0),
        ("TREND_FOLLOWING", 8, 21, 14, 52.0, 48.0, 1.5, 5.0, 0.02, 2, "RR_DYNAMIC", "ATR_REGIME", 0),
        ("TREND_FOLLOWING", 9, 26, 14, 50.0, 50.0, 1.5, 5.5, 0.015, 3, "RR_DYNAMIC", None, 0),
        ("TREND_FOLLOWING", 10, 30, 14, 50.0, 50.0, 1.5, 6.0, 0.015, 3, "RR_DYNAMIC", "ATR_REGIME", 0),
        ("TREND_FOLLOWING", 12, 36, 14, 53.0, 47.0, 1.8, 7.0, 0.015, 3, "RR_DYNAMIC", "ATR_REGIME", 0),
        ("TREND_FOLLOWING", 15, 45, 14, 55.0, 45.0, 2.0, 7.5, 0.015, 3, "RR_DYNAMIC", None, 0),

        # RSI_MOMENTUM
        ("RSI_MOMENTUM", 5, 15, 9, 50.0, 50.0, 1.2, 4.0, 0.02, 2, "RR_DYNAMIC", None, 0),
        ("RSI_MOMENTUM", 6, 18, 9, 52.0, 48.0, 1.2, 4.5, 0.02, 2, "RR_DYNAMIC", None, 0),
        ("RSI_MOMENTUM", 8, 24, 14, 55.0, 45.0, 1.5, 5.5, 0.015, 2, "RR_DYNAMIC", "ATR_REGIME", 0),
        ("RSI_MOMENTUM", 10, 30, 14, 50.0, 50.0, 1.5, 6.0, 0.02, 3, "RR_DYNAMIC", None, 0),
        ("RSI_MOMENTUM", 12, 36, 14, 54.0, 46.0, 1.8, 7.0, 0.015, 3, "RR_DYNAMIC", "ATR_REGIME", 0),

        # EMA_CROSS & MEAN_REVERSION
        ("EMA_CROSS", 6, 18, 9, 50.0, 50.0, 1.2, 4.0, 0.02, 2, "RR_DYNAMIC", None, 0),
        ("EMA_CROSS", 10, 30, 14, 50.0, 50.0, 1.5, 5.5, 0.015, 3, "RR_DYNAMIC", None, 0),
        ("MEAN_REVERSION", 8, 21, 10, 65.0, 35.0, 1.5, 4.0, 0.02, 2, "RR_DYNAMIC", None, 0),
        ("MEAN_REVERSION", 10, 30, 14, 70.0, 30.0, 1.8, 4.5, 0.015, 2, "RR_DYNAMIC", None, 0),
    ]


def process_dataset_task(dataset_info):
    data_file, m_file, symbol, timeframe, mdata = dataset_info
    certified_items = []

    file_size_mb = os.path.getsize(data_file) / (1024 * 1024)
    if file_size_mb > 30.0:
        return certified_items
    ...
    search_configs = get_crypto_search_configs()
    ...
    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, exit_fam, vol_filt, brk_lk) in enumerate(search_configs, 1):
        strat_id = f"UR_ULTRA_{symbol.upper()}_{timeframe.upper()}_q{cfg_idx}"
        ...
        if (oos_bt.profit_factor >= min_pf_limit 
            ...
            if verdict.is_certified or passed_count == 11:
                ...
                save_candidate_to_sqlite(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso)
                ...
                print(f"   🏆 ¡CERTIFICADA 11/11! {snapshot.strategy_id} | {symbol} {timeframe} | PF={oos_bt.profit_factor:.2f} | DD={oos_bt.max_drawdown_pct:.2f}% | Trades={oos_bt.total_trades} | Return={monthly_roi_pct:.1f}%/mes", flush=True)
...
def main():
    print("==========================================================================", flush=True)
    print("⚡ MINERÍA CUANTITATIVA Y CERTIFICACIÓN 11/11 ULTRA CRYPTO CON PIRAMIDACIÓN", flush=True)
    print("==========================================================================", flush=True)
        ...
        try:
            ...
            if sym in TARGET_SYMBOLS and tf in TARGET_TFS:
                ...
                if os.path.exists(df):
                    datasets.append((df, m_file, sym, tf, mdata))
        except Exception as e:
            print(f"Error manifest {m_file}: {e}", flush=True)

    print(f"📊 Datasets Cripto objetivo (SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI x 1m, 5m, 15m, 1h, 4h): {len(datasets)}", flush=True)

    all_certified = []

    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process_dataset_task, ds): ds for ds in datasets}
        for future in as_completed(futures):
            ...
            try:
                ...
                if res:
                    ...
                    print(f"🎯 Completed dataset {sym} ({tf}): certified {len(res)} strategy(ies)", flush=True)
                else:
                    print(f"ℹ️ Completed dataset {sym} ({tf}): 0 passing 11/11", flush=True)
            except Exception as e:
                print(f"❌ Worker error on {sym} ({tf}): {e}", flush=True)

    print("==========================================================================", flush=True)
    print(f"🏁 MINERÍA FINALIZADA. TOTAL ESTRATEGIAS CERTIFICADAS ULTRA CRYPTO 11/11: {len(all_certified)}", flush=True)
    print("==========================================================================", flush=True)

    for item in all_certified:
        print(f"  🏆 {item['strategy_id']} | {item['symbol']} {item['timeframe']} | PF={item['pf_oos']:.2f} | DD={item['max_dd_oos']:.2f}% | Trades={item['trades_oos']} | ROI/m={item['monthly_roi_pct']}% | Hash={item['canonical_hash'][:16]}...", flush=True)
