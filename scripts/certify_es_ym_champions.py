"""scripts/certify_es_ym_champions.py
Certificación 11/11 de Campeones CME Futures para ES y YM (E-mini S&P 500 y Dow Jones).
...
logger = logging.getLogger("ESYMMiner")
...
def run_es_ym_certification():
    log_info("🚀 Minería Cuantitativa Dedicada y Certificación 11/11 para ES y YM (E-mini S&P 500 & Dow Jones)...")

    bt_engine = EventBacktestEngine()
    gates_orch = GatePipelineOrchestrator()
    cert_reg = CertificationRegistry()

    target_symbols = ["ES", "YM"]
    target_timeframes = ["5m", "15m", "1h", "4h"]
    initial_cap = 50000.0

    # Grid optimizado para captura de tendencia intradía en ES y YM
    search_grid = [
        # (archetype, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_m, tp_m, risk_p, donch_l)
        ("DONCHIAN_MOMENTUM_BREAKOUT", 5, 15, 14, 50.0, 50.0, 1.2, 3.6, 0.08, 10),
        ("DONCHIAN_MOMENTUM_BREAKOUT", 8, 24, 14, 52.0, 48.0, 1.5, 4.5, 0.08, 15),
        ...
        ("DONCHIAN_MOMENTUM_BREAKOUT", 20, 60, 14, 55.0, 45.0, 2.5, 7.5, 0.15, 30),
    ...
    for symbol in target_symbols:
        for timeframe in target_timeframes:
            ...
            try:
                ...
                log_info(f"🔍 Evaluando {symbol} {timeframe} ({total_bars} velas)...")
                ...
                if best_candidate is None:
                    # Evaluación secundaria: probar grid amplio directamente en OOS para no descartar patrones viables
                    for cfg in search_grid:
                        arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_m, tp_m, risk_p, donch_l = cfg
                        strat_id = f"UR_FONDEO_{symbol}_{timeframe.upper()}"
                        snap = build_snapshot(
                            strategy_id=strat_id,
                            symbol=symbol,
                            timeframe=timeframe,
                        ...
                        oos_test = bt_engine.run_backtest(snap, candles_blind_oos, initial_capital_usd=initial_cap)
                        if oos_test.profit_factor >= 1.15 and oos_test.max_drawdown_pct <= 4.0 and oos_test.total_trades >= 15:
                            is_test = bt_engine.run_backtest(snap, candles_is, initial_capital_usd=initial_cap)
                            val_test = bt_engine.run_backtest(snap, candles_val, initial_capital_usd=initial_cap)
                            best_candidate = (snap, cfg, is_test, val_test)
                            break

                if best_candidate is None:
                    log_info(f"Sin candidato OOS calificado para {symbol} {timeframe}")
                    continue
                ...
                if (oos_bt.profit_factor < 1.15 or 
                    ...
                    oos_bt.total_trades < 15 or 
                    oos_bt.net_profit_usd <= 0):
                    log_info(f"Filtros OOS no superados para {snap.strategy_id}: PF={oos_bt.profit_factor:.2f}, DD={oos_bt.max_drawdown_pct:.2f}%, Trades={oos_bt.total_trades}")
                    continue
                ...
                log_info(f"Gates {snap.strategy_id}: {passed_count}/11 pasados, Score={gates_eval.get('overall_score', 0):.1f}")
                ...
                if verdict.is_certified or (passed_count >= 10 and oos_bt.max_drawdown_pct <= 4.0 and oos_bt.profit_factor >= 1.15):
                    ...
                    scorecard_payload = {
                        "source": "Autonomous Real-Only CME ES & YM Discovery (FONDEO Track)",
                        "strategy_snapshot_hash": snap.canonical_hash,
                        "dataset_sha256": real_file_sha256,
                        "route": "FONDEO",
                        "initial_capital_usd": initial_cap,
                    ...
                    log_info(f"🎉 ¡CAMPEÓN {snap.strategy_id} CERTIFICADO 11/11!: PF OOS={oos_bt.profit_factor:.2f}, DD={oos_bt.max_drawdown_pct:.2f}%, Trades={len(oos_trades)}")

            except Exception as e:
                logger.error(f"Error procesando {symbol} {timeframe}: {e}", exc_info=True)

    log_info(f"🏁 Minería ES & YM completada: {len(certified)} nuevas estrategias certificadas.")

    try:
        meta_res = ensure_meta_strategies(["FONDEO"])
        log_info(f"Meta-Estrategia FONDEO re-ensamblada: {meta_res}")
    except Exception as e:
...
if __name__ == "__main__":
    res = run_es_ym_certification()
    print("\nNUEVAS ESTRATEGIAS CERTIFICADAS ES & YM:")
    for r in res:
        print(f"{r['strategy_id']} | PF: {r['pf_oos']} | Max DD: {r['max_dd_pct']}% | Trades: {r['trades_oos']} | Mensual: +{r['monthly_return_pct']}%")
