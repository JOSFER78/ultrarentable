...
class Gate04WalkForward(GateBase):
    GATE_ID = 4
    NAME = "WALK_FORWARD"
    ...
    def evaluar(self, ev: Evidencia) -> GateResult:
        return self._resultado(
            self.evaluate(
                ev.pre_oos_trades or ev.is_trades or [],
            )
        )
    ...
    def evaluate(
        ...
        if not trades_pnl or len(trades_pnl) < self.UMBRALES["min_trades"]:
            return {
        ...
        passed = (avg_wfe >= self.UMBRALES["min_avg_wfe"]) and (consistency_pct >= self.UMBRALES["min_consistency_pct"])
