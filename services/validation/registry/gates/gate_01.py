...
class Gate01DataIngest(GateBase):
    GATE_ID = 1
    NAME = "DATA_INGEST"
    ...
    def evaluar(self, ev: Evidencia) -> GateResult:
        return self._resultado(
            self.evaluate(
                ev.candles or [],
                timeframe=ev.candidate_info.get("timeframe", "1h"),
                dataset_filepath=ev.candidate_info.get("dataset_filepath"),
            )
        )
    ...
    def evaluate(
        ...
        min_required = (
            self.UMBRALES["min_bars_1d"]
            if tf_clean in ("1d", "d1")
            else (
                self.UMBRALES["min_bars_4h"]
                if tf_clean in ("4h", "h4")
                else (
                    self.UMBRALES["min_bars_1h"]
                    if tf_clean in ("1h", "h1")
                    else self.UMBRALES["min_bars_otros"]
                )
            )
        )
        ...
        passed = (corrupt_bars == 0) and (out_of_order_bars == 0) and (total_candles >= min_required) and (gaps_detected <= total_candles * self.UMBRALES["max_gap_frac"])
