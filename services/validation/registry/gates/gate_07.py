...
class Gate07RegimeCoverage(GateBase):
    GATE_ID = 7
    ...
    def evaluar(self, ev: Evidencia) -> GateResult:
        return self._resultado(
            self.evaluate(
                candles=ev.candles or [],
                trades_raw=ev.trades_raw or [],
    ...
    def evaluate(
        ...
        if not candles or len(candles) < self.UMBRALES["min_candles"]:
            return {
        ...
        min_active_required = self.UMBRALES["min_active_regimes"]
