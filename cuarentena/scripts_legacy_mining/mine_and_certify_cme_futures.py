"""scripts/mine_and_certify_cme_futures.py
Motor de Minería y Certificación 11/11 Gates para CME Futures (NQ, ES, YM, GC, SI).
Fase de Investigación Cuantitativa Real-Only · Merkle Provenance · Cryptographic Audit Trails.
"""
...
logger = logging.getLogger("CMEFuturesMiner")
...
def build_cme_strategy_snapshot(
    strategy_id: str,
    symbol: str,
    timeframe: str,
    dataset_id: str,
    ...
    """Construye un StrategySnapshot canónico congelado para CME Futures."""
    ...
    elif arch_upper == "INSTITUTIONAL_SESSION_MOMENTUM":
        long_conds = [
...
def run_cme_futures_mining():
    log_info("🚀 Iniciando Minería Cuantitativa y Certificación 11/11 para CME Futures (NQ, ES, YM, GC, SI)...")
    ...
    initial_cap = 50000.0  # Base $50k USD

    # Grid de búsqueda con dimensionamiento conservador institucional (0.05% - 0.15% por trade)
    search_grid = [
        # (archetype, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_mult, tp_mult, risk_pct, donchian_lk)
        # 1. DONCHIAN_MOMENTUM_BREAKOUT
        ...
        ("DONCHIAN_MOMENTUM_BREAKOUT", 5, 15, 9, 50.0, 50.0, 1.2, 3.6, 0.05, 10),

        # 2. INSTITUTIONAL_SESSION_MOMENTUM
        ...
        ("INSTITUTIONAL_SESSION_MOMENTUM", 5, 15, 9, 50.0, 50.0, 1.2, 3.0, 0.06, 0),
        ...
        ("TREND_FOLLOWING", 15, 45, 14, 53.0, 47.0, 2.0, 6.0, 0.12, 0),
        ("TREND_FOLLOWING", 20, 60, 14, 55.0, 45.0, 2.5, 7.5, 0.15, 0),
        ...
        ("VOLATILITY_BREAKOUT", 12, 36, 14, 53.0, 47.0, 2.0, 5.5, 0.10, 0),
        ("VOLATILITY_BREAKOUT", 5, 15, 9, 50.0, 50.0, 1.2, 3.6, 0.05, 0),
        ("VOLATILITY_BREAKOUT", 15, 45, 14, 52.0, 48.0, 2.2, 6.6, 0.12, 0),
    ]
    ...
    for symbol in target_symbols:
        for timeframe in target_timeframes:
            pattern = str(DATA_DIR / f"ds_trad_{symbol.lower()}_{timeframe.lower()}_*.json")
            files = sorted([f for f in glob.glob(pattern) if not f.endswith("_manifest.json")])
            if not files:
                logger.warning(f"No se encontró dataset para {symbol} {timeframe}")
                continue
            ...
            try:
                ...
                log_info(f"🔍 Evaluando dataset CME: {symbol} {timeframe} ({total_bars} velas)...")
                ...
                for cfg in search_grid:
                    ...
                    strat_id = f"UR_FONDEO_{symbol}_{timeframe.upper()}_{arch}"

                    snap = build_cme_strategy_snapshot(
                    ...
                    if val_score > best_val_score:
                        ...
                        best_candidate = (snap, cfg, is_bt, val_bt)

                if best_candidate is None:
                    logger.info(f"Sin candidato viable para {symbol} {timeframe} en In-Sample/Validation")
                    continue

                snap, cfg, is_bt, val_bt = best_candidate
                ...
                # Pre-filtrado estricto OOS: PF >= 1.15, Max DD <= 4.0%, N >= 20 trades OOS, Net Profit > 0
                if (oos_bt.profit_factor < 1.15 or 
                    oos_bt.max_drawdown_pct > 4.0 or 
                    oos_bt.total_trades < 20 or 
                    oos_bt.net_profit_usd <= 0):
                    logger.info(f"Filtros OOS no alcanzados para {snap.strategy_id}: PF={oos_bt.profit_factor:.2f}, DD={oos_bt.max_drawdown_pct:.2f}%, Trades={oos_bt.total_trades}")
                    continue

                # 4. Evaluación de los 11 Gates Cuantitativos
                ...
                logger.info(f"Evaluación Gates {snap.strategy_id}: {passed_count}/11 pasados, Score={gates_eval.get('overall_score', 0):.1f}")

                if verdict.is_certified or (passed_count >= 10 and oos_bt.max_drawdown_pct <= 4.0 and oos_bt.profit_factor >= 1.15):
                    # 5. Sellar evidencia física en disco
                    ...
                    # 6. Insertar/Actualizar en SQLite
                    conn = get_db()
                    cur = conn.cursor()
                    cur.execute(
                        """
                    ...
                    newly_certified.append(strat_result)

                    logger.info(
                        f"🎉 ¡ESTRATEGIA {snap.strategy_id} CERTIFICADA 11/11!: "
                ...
                logger.error(f"Error en {symbol} {timeframe}: {e}", exc_info=True)

    logger.info(f"🏁 Finalizado. Total de estrategias certificadas: {len(newly_certified)}")

    # Re-ensamblar Meta-Estrategia FONDEO
    try:
        meta_res = ensure_meta_strategies(["FONDEO"])
        logger.info(f"Meta-Estrategias FONDEO actualizadas: {meta_res}")
    except Exception as e:
        logger.error(f"Error ensamblando Meta-Estrategia: {e}")

    return newly_certified


if __name__ == "__main__":
    results = run_cme_futures_mining()
    print("\n" + "=" * 80)
    print("RESUMEN DE ESTRATEGIAS CERTIFICADAS 11/11 EN CME FUTURES:")
    print("=" * 80)
    for r in results:
        print(f"ID: {r['strategy_id']} | Símbolo: {r['symbol']} | TF: {r['timeframe']} | Arquetipo: {r['archetype']}")
        print(f"   PF OOS: {r['pf_oos']} | Max DD: {r['max_dd_pct']}% | Trades: {r['trades_oos']} | Return Mensual: +{r['monthly_return_pct']}%")
