"""scripts/fast_intraday_miner.py
Fast, Comprehensive Quantitative Miner & 11/11 Certifier for ULTRA Intraday Strategies with Pyramiding.
...
def save_to_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        """
...
def mine_and_certify():
    print("==========================================================================", flush=True)
    print("⚡ MINERÍA Y CERTIFICACIÓN 11/11 ULTRA INTRADÍA DESATENDIDA", flush=True)
    print("==========================================================================", flush=True)
    ...
    print(f"📊 Total datasets intradía a examinar: {len(intraday_datasets)}", flush=True)
    ...
    # High payoff ratio & Taleb convexity grid with active pyramiding
    search_grid = [
    ...
    for ds_idx, (data_file, m_file, symbol, timeframe, mdata) in enumerate(intraday_datasets, 1):
        ...
        if size_mb > 35.0:
            print(f"[{ds_idx}/{len(intraday_datasets)}] Omitiendo dataset masivo (>35MB): {symbol} {timeframe}", flush=True)
            continue
        ...
        print(f"[{ds_idx}/{len(intraday_datasets)}] Evaluando {symbol} ({timeframe}) - {total_bars} velas ({len(candles_blind_oos)} OOS)", flush=True)
        ...
        for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk) in enumerate(search_grid, 1):
            ...
            if (oos_bt.profit_factor >= min_pf_limit 
                ...
                if verdict.is_certified or passed_count == 11:
                    ...
                    save_to_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso)
                    certified_strategies.append(snapshot.strategy_id)
                    print(f"  🏆 ¡ESTRATEGIA CERTIFICADA 11/11 REGISTRADA! -> {snapshot.strategy_id} ({symbol} {timeframe}, PF: {oos_bt.profit_factor:.2f}, DD: {oos_bt.max_drawdown_pct:.2f}%, Trades: {oos_bt.total_trades})", flush=True)

    print("==========================================================================", flush=True)
    print(f"🏁 MINERÍA Y CERTIFICACIÓN FINALIZADA. CERTIFICADAS 11/11 REGISTRADAS: {len(certified_strategies)}", flush=True)
    print("==========================================================================", flush=True)
