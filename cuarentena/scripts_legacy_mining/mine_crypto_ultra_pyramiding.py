"""scripts/mine_crypto_ultra_pyramiding.py
Motor de Minería y Certificación 11/11 ULTRA Intradía para Criptoactivos con Piramidación Activa.
Soporta Símbolos: SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI.
Marcos Temporales: 1m, 5m, 15m, 1h, 4h.
Garantiza 100% Real-Only, Cero Mocks, Trazabilidad Criptográfica SHA-256.
"""
...
logger = logging.getLogger("CryptoUltraMiner")
...
def save_certified_candidate(snapshot, symbol, timeframe, fname, file_sha256, initial_cap, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute(
        ...
        logger.error(f"❌ Error guardando candidato en SQLite: {e}")
        return False


def build_search_space():
    """Genera configuraciones cuantitativas enfocadas en asimetría y piramidación 2-3 tiers."""
    configs = []
    
    # Archetype 1: MOMENTUM_BREAKOUT (Donchian breakout + Trend filter)
    # (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk, exit_fam)
    for brk_lk in [10, 15, 20, 25]:
        for py in [2, 3]:
            for tp_mult in [4.0, 5.0, 6.0, 7.5]:
                for sl_mult in [1.2, 1.5, 1.8]:
                    configs.append(("MOMENTUM_BREAKOUT", 8, 21, 14, 52.0, 48.0, sl_mult, tp_mult, 0.02, py, None, brk_lk, "RR_DYNAMIC"))
                    configs.append(("MOMENTUM_BREAKOUT", 10, 30, 14, 50.0, 50.0, sl_mult, tp_mult, 0.015, py, "ATR_REGIME", brk_lk, "RR_DYNAMIC"))

    # Archetype 2: TREND_FOLLOWING (EMA Cross + ATR Filter)
    for ema_f, ema_s in [(5, 20), (8, 21), (10, 30), (12, 36), (15, 45)]:
        for py in [2, 3]:
            for tp_mult in [4.5, 5.5, 6.5, 8.0]:
                for sl_mult in [1.2, 1.5, 2.0]:
                    configs.append(("TREND_FOLLOWING", ema_f, ema_s, 14, 52.0, 48.0, sl_mult, tp_mult, 0.02, py, "ATR_REGIME", 0, "RR_DYNAMIC"))
                    configs.append(("TREND_FOLLOWING", ema_f, ema_s, 14, 50.0, 50.0, sl_mult, tp_mult, 0.015, py, None, 0, "RR_DYNAMIC"))

    # Archetype 3: RSI_MOMENTUM (EMA Direction + RSI Threshold Momentum)
    for rsi_l, rsi_s in [(52.0, 48.0), (55.0, 45.0), (50.0, 50.0)]:
        for py in [2, 3]:
            for tp_mult in [4.0, 5.0, 6.0]:
                for sl_mult in [1.2, 1.5, 1.8]:
                    configs.append(("RSI_MOMENTUM", 8, 24, 14, rsi_l, rsi_s, sl_mult, tp_mult, 0.02, py, None, 0, "RR_DYNAMIC"))
                    configs.append(("RSI_MOMENTUM", 12, 36, 14, rsi_l, rsi_s, sl_mult, tp_mult, 0.015, py, "ATR_REGIME", 0, "RR_DYNAMIC"))

    return configs


def evaluate_crypto_dataset(dataset_tuple):
    data_file, m_file, symbol, timeframe, mdata = dataset_tuple
    certified_results = []

    # Cargar datos
    size_mb = os.path.getsize(data_file) / (1024 * 1024)
    if size_mb > 35.0:  # Evitar archivos gigantes de 1m en procesos individuales
    ...
    initial_cap = 1000.0
    max_dd_limit = 30.0  # Subcuenta bala ULTRA max DD <= 30%
    min_pf_limit = 1.10
    ...
    search_grid = build_search_space()
    trials_count = len(search_grid)

    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk, exit_fam) in enumerate(search_grid, 1):
        ...
        try:
            ...
            if (oos_bt.profit_factor >= min_pf_limit 
                ...
                if verdict.is_certified or passed_count == 11:
                    # Persistencia física en evidencia
                    ...
                    save_certified_candidate(snapshot, symbol, timeframe, fname, file_sha256, initial_cap, is_bt, oos_bt, scorecard_payload, certified_at_iso)
                    ...
                    certified_results.append(item_data)
                    logger.info(f"🏆 CERTIFICADA 11/11 CRYPTO: {snapshot.strategy_id} | {symbol} {timeframe} | PF={oos_bt.profit_factor:.2f} | DD={oos_bt.max_drawdown_pct:.2f}% | Return={monthly_roi_pct:.1f}%/mes")

        except Exception as e:
            logger.debug(f"Error evaluando config {cfg_idx} en {symbol} {timeframe}: {e}")
...
def main():
    logger.info("==========================================================================")
    logger.info("⚡ INICIANDO MINERÍA Y CERTIFICACIÓN 11/11 ULTRA CRYPTO INTRADÍA")
    logger.info("==========================================================================")
    ...
    for m_file in manifest_files:
        try:
            ...
            if sym in TARGET_SYMBOLS and tf in TARGET_TFS:
                df = m_file.replace("_manifest.json", ".json")
                if os.path.exists(df):
                    crypto_datasets.append((df, m_file, sym, tf, mdata))
        except Exception as e:
            logger.error(f"⚠️ Error cargando manifiesto {m_file}: {e}")

    logger.info(f"📊 Datasets Cripto Identificados: {len(crypto_datasets)}")

    all_certified = []

    # Ejecutar secuencialmente o con pool controlado
    for idx, ds in enumerate(crypto_datasets, 1):
        sym = ds[2]
        tf = ds[3]
        logger.info(f"[{idx}/{len(crypto_datasets)}] Procesando {sym} ({tf})...")
        res = evaluate_crypto_dataset(ds)
        if res:
            all_certified.extend(res)
            logger.info(f"   -> {len(res)} estrategia(s) aprobada(s) 11/11 para {sym} {tf}")

    logger.info("==========================================================================")
    logger.info(f"🏁 MINERÍA COMPLETADA. TOTAL ESTRATEGIAS CERTIFICADAS 11/11: {len(all_certified)}")
    logger.info("==========================================================================")
    for c in all_certified:
        logger.info(f"   🏆 {c['strategy_id']} | {c['symbol']} {c['timeframe']} | PF={c['pf_oos']:.2f} | DD={c['max_dd_oos']:.2f}% | Trades={c['trades_oos']} | ROI/m={c['monthly_roi_pct']}%")
