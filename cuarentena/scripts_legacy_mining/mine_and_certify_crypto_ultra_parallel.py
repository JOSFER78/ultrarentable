"""scripts/mine_and_certify_crypto_ultra_parallel.py
Motor de Minería y Certificación 11/11 ULTRA Intradía Multiproceso para Criptoactivos con Piramidación Activa.
Símbolos Objetivo: SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI.
...
logger = logging.getLogger("CryptoUltraParallelMiner")
...
LOG_FILE = EVIDENCE_DIR / "crypto_ultra_mining.log"
...
def save_certified_candidate_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute(
        ...
        log_msg(f"❌ Error guardando candidato en DB SQLite: {e}")
        return False


def build_search_grid():
    """Grid optimizado de alta efectividad para asimetría y piramidación intradía (2-3 tiers)."""
    grid = []
    
    # 1. TREND_FOLLOWING con filtro ATR y piramidación 2 o 3 tiers
    for ema_f, ema_s in [(5, 20), (8, 21), (10, 30), (12, 36), (15, 45)]:
        for sl_m, tp_m in [(1.2, 4.5), (1.5, 5.0), (1.5, 6.0), (1.8, 7.0), (2.0, 7.5)]:
            for py in [2, 3]:
                for risk_p in [0.015, 0.02]:
                    grid.append(("TREND_FOLLOWING", ema_f, ema_s, 14, 52.0, 48.0, sl_m, tp_m, risk_p, py, "ATR_REGIME", 0))
                    grid.append(("TREND_FOLLOWING", ema_f, ema_s, 14, 50.0, 50.0, sl_m, tp_m, risk_p, py, None, 0))

    # 2. MOMENTUM_BREAKOUT con Donchian Lookback y piramidación 2 o 3 tiers
    for brk_lk in [10, 15, 20, 25]:
        for ema_f, ema_s in [(8, 21), (10, 30), (15, 40)]:
            for sl_m, tp_m in [(1.2, 4.5), (1.5, 5.5), (1.8, 6.5), (2.0, 8.0)]:
                for py in [2, 3]:
                    grid.append(("MOMENTUM_BREAKOUT", ema_f, ema_s, 14, 52.0, 48.0, sl_m, tp_m, 0.02, py, "ATR_REGIME", brk_lk))
                    grid.append(("MOMENTUM_BREAKOUT", ema_f, ema_s, 14, 50.0, 50.0, sl_m, tp_m, 0.015, py, None, brk_lk))

    # 3. RSI_MOMENTUM con piramidación 2 o 3 tiers
    for rsi_l, rsi_s in [(52.0, 48.0), (55.0, 45.0), (50.0, 50.0)]:
        for ema_f, ema_s in [(6, 18), (8, 24), (12, 36)]:
            for sl_m, tp_m in [(1.2, 4.5), (1.5, 5.5), (1.8, 6.5)]:
                for py in [2, 3]:
                    grid.append(("RSI_MOMENTUM", ema_f, ema_s, 14, rsi_l, rsi_s, sl_m, tp_m, 0.02, py, "ATR_REGIME", 0))
                    grid.append(("RSI_MOMENTUM", ema_f, ema_s, 14, rsi_l, rsi_s, sl_m, tp_m, 0.015, py, None, 0))

    return grid


def process_crypto_dataset(dataset_tuple):
    ...
    initial_cap = 1000.0
    max_dd_limit = 30.0  # Drawdown subcuenta bala <= 30.0%
    min_pf_limit = 1.10
    min_trades_limit = 10
    ...
    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk) in enumerate(search_grid, 1):
        ...
        if (oos_bt.profit_factor >= min_pf_limit 
            ...
            candidate_info = {
                ...
                "dataset_sha256": file_sha256,
                "trials_tested": trials_tested_count,
                "parameters": {
                    "ema_fast": ema_f,
                    "ema_slow": ema_s,
                    "rsi_period": rsi_p,
            ...
            if verdict.is_certified or passed_count == 11:
                # Sellado físico de evidencia en data/evidence/<strategy_id>/ledger_oos.json
                ...
                save_certified_candidate_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso)
                certified_list.append(item_res)
...
def main():
    log_msg("==========================================================================")
    log_msg("⚡ MOTOR MASTER MULTIPROCESO DE MINERÍA Y CERTIFICACIÓN 11/11 ULTRA CRYPTO")
    log_msg("==========================================================================")
    ...
    for m_file in manifest_files:
        try:
            ...
            log_msg(f"⚠️ Error cargando manifiesto {m_file}: {e}")

    log_msg(f"📊 Total datasets cripto identificados (SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI x 1m, 5m, 15m, 1h, 4h): {len(crypto_datasets)}")

    all_certified = []

    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process_crypto_dataset, ds): ds for ds in crypto_datasets}
        for future in as_completed(futures):
            ...
            try:
                ...
                if res_list:
                    all_certified.extend(res_list)
                    for item in res_list:
                        snap = item["snapshot"]
                        bt = item["oos_bt"]
                        roi = item["monthly_roi_pct"]
                        log_msg(f"🎯 CERTIFICADA 11/11: {snap.strategy_id} | {sym} {tf} | PF={bt.profit_factor:.2f} | DD={bt.max_drawdown_pct:.2f}% | Trades={bt.total_trades} | ROI/m={roi}% | PnL=${bt.net_profit_usd:.2f}")
            except Exception as e:
                log_msg(f"❌ Error en worker dataset {sym} ({tf}): {e}")

    log_msg("==========================================================================")
    log_msg(f"🏁 PROCESO DE CERTIFICACIÓN COMPLETADO. TOTAL ESTRATEGIAS CERTIFICADAS ULTRA CRYPTO: {len(all_certified)}")
    log_msg("==========================================================================")
