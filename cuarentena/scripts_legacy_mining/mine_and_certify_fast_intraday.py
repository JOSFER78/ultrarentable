"""scripts/mine_and_certify_fast_intraday.py
Minería y Certificación 11/11 Ultrarrápida de Estrategias ULTRA Intradía con Piramidación.
Cubre Marcos Temporales: 1m, 5m, 15m, 1h para el Universo Cripto Global (BTC, ETH, SOL, SUI, XRP, BNB, AVAX, LINK, DOGE).
...
def evaluate_dataset(args):
    ...
    # High payoff ratio & Taleb convexity search grid with active pyramiding
    search_grid = [
    ...
    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, vol_filt, brk_lk) in enumerate(search_grid, 1):
        strat_id = f"UR_ULTRA_{symbol.upper()}_{timeframe.upper()}_fast_v{cfg_idx}"
...
def main():
    print("==========================================================================", flush=True)
    print("⚡ MINERÍA Y CERTIFICACIÓN MULTIPROCESO 11/11 ULTRA INTRADÍA", flush=True)
    print("==========================================================================", flush=True)
    ...
    print(f"📊 Total datasets a evaluar: {len(intraday_datasets)}", flush=True)

    all_certified = []
    
    # Procesar en paralelo
    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(evaluate_dataset, ds) for ds in intraday_datasets]
    ...
    print(f"💾 Guardando {len(all_certified)} estrategias certificadas en SQLite...", flush=True)
    conn = get_db()
    cur = conn.cursor()
    ...
    for item in all_certified:
        ...
        print(f"   ✅ REGISTRADA EN SQLITE -> {snapshot.strategy_id} ({symbol} {timeframe}, PF: {oos_bt.profit_factor:.2f}, DD: {oos_bt.max_drawdown_pct:.2f}%)", flush=True)

    conn.commit()
    conn.close()

    print("==========================================================================", flush=True)
    print(f"🏁 PROCESO FINALIZADO. TOTAL ESTRATEGIAS CERTIFICADAS REGISTRADAS 11/11: {len(all_certified)}", flush=True)
    print("==========================================================================", flush=True)
