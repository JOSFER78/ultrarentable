"""scripts/mine_and_certify_all_crypto_perfect.py
Motor Definitorio de Minería Cuantitativa y Certificación 11/11 ULTRA Intradía para Criptoactivos con Piramidación Activa.
Garantiza Cobertura Real 100%: SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI en 1m, 5m, 15m, 1h, 4h.
...
LOG_FILE = EVIDENCE_DIR / "crypto_perfect_mining.log"
...
def save_certified_candidate_to_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute(
...
def build_crypto_search_space():
    """Genera una matriz cuantitativa amplia enfocada en asimetría y piramidación 2-3 tiers."""
    configs = []

    # 1. TREND_FOLLOWING (EMA Cross + ATR Regime filter)
    for ema_f, ema_s in [(4, 12), (5, 15), (6, 18), (8, 21), (10, 30), (12, 36), (15, 45)]:
        for sl_m, tp_m in [(1.0, 3.5), (1.2, 4.0), (1.2, 4.5), (1.5, 5.0), (1.5, 6.0), (1.8, 7.0)]:
            for py in [2, 3]:
                for risk_p in [0.015, 0.02]:
                    configs.append(("TREND_FOLLOWING", ema_f, ema_s, 14, 52.0, 48.0, sl_m, tp_m, risk_p, py, "RR_DYNAMIC", "ATR_REGIME", 0))
                    configs.append(("TREND_FOLLOWING", ema_f, ema_s, 14, 50.0, 50.0, sl_m, tp_m, risk_p, py, "RR_DYNAMIC", None, 0))

    # 2. MOMENTUM_BREAKOUT (Donchian breakout + EMA Trend)
    for brk_lk in [10, 15, 20, 25]:
        for ema_f, ema_s in [(5, 15), (8, 21), (10, 30), (12, 36)]:
            for sl_m, tp_m in [(1.2, 4.0), (1.5, 5.0), (1.6, 6.0), (1.8, 7.0)]:
                for py in [2, 3]:
                    configs.append(("MOMENTUM_BREAKOUT", ema_f, ema_s, 14, 52.0, 48.0, sl_m, tp_m, 0.02, py, "RR_DYNAMIC", "ATR_REGIME", brk_lk))
                    configs.append(("MOMENTUM_BREAKOUT", ema_f, ema_s, 14, 50.0, 50.0, sl_m, tp_m, 0.015, py, "RR_DYNAMIC", None, brk_lk))

    # 3. RSI_MOMENTUM
    for rsi_l, rsi_s in [(52.0, 48.0), (55.0, 45.0), (50.0, 50.0)]:
        for ema_f, ema_s in [(4, 12), (6, 18), (8, 24), (10, 30)]:
            for sl_m, tp_m in [(1.0, 3.5), (1.2, 4.2), (1.5, 5.5), (1.8, 6.5)]:
                for py in [2, 3]:
                    configs.append(("RSI_MOMENTUM", ema_f, ema_s, 14, rsi_l, rsi_s, sl_m, tp_m, 0.02, py, "RR_DYNAMIC", "ATR_REGIME", 0))
                    configs.append(("RSI_MOMENTUM", ema_f, ema_s, 14, rsi_l, rsi_s, sl_m, tp_m, 0.015, py, "RR_DYNAMIC", None, 0))

    return configs


def process_single_dataset(dataset_tuple):
    ...
    initial_cap = 1000.0
    max_dd_limit = 30.0  # Subcuenta bala ULTRA max DD <= 30.0%
    min_pf_limit = 1.10
    min_trades_limit = 10
    ...
    search_configs = build_crypto_search_space()
    trials_tested_count = len(search_configs)
    ...
    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, exit_fam, vol_filt, brk_lk) in enumerate(search_configs, 1):
        strat_id = f"UR_ULTRA_{symbol.upper()}_{timeframe.upper()}_p{cfg_idx}"
        ...
        if (oos_bt.profit_factor >= min_pf_limit 
            ...
            if verdict.is_certified or passed_count == 11:
                ...
                save_certified_candidate_to_db(snapshot, symbol, timeframe, fname, is_bt, oos_bt, scorecard_payload, certified_at_iso)
...
def main():
    log_msg("==========================================================================")
    log_msg("⚡ MOTOR PERFECTO DE MINERÍA Y CERTIFICACIÓN 11/11 ULTRA CRYPTO")
    log_msg("==========================================================================")
    ...
    dataset_list = [(df, mf, s, t, md) for (sz, df, mf, s, t, md) in best_datasets.values()]
    log_msg(f"📊 Datasets Cripto Óptimos Seleccionados: {len(dataset_list)} (Símbolos: {TARGET_SYMBOLS})")

    all_certified = []

    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process_single_dataset, ds): ds for ds in dataset_list}
            ...
            try:
                ...
                if res_list:
                    ...
                    for item in res_list:
                        ...
                        log_msg(f"🎯 CERTIFICADA 11/11: {s_id} | {sym} {tf} | PF={pf:.2f} | DD={dd:.2f}% | Trades={tr} | ROI/m={roi}%")
                else:
                    log_msg(f"ℹ️ Dataset finalizado {sym} ({tf}): 0 certificadas en este grid")
            except Exception as e:
                log_msg(f"❌ Error en worker dataset {sym} ({tf}): {e}")

    log_msg("==========================================================================")
    log_msg(f"🏁 PROCESO COMPLETADO. TOTAL ESTRATEGIAS CERTIFICADAS ULTRA CRYPTO 11/11: {len(all_certified)}")
    log_msg("==========================================================================")
