"""scripts/fast_certifier.py
Motor Hyper-Rápido de Certificación Cuantitativa ULTRA Intradía 11/11 para Criptoactivos con Piramidación Activa (2-3 Tiers).
Cubre: SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI en 1m, 5m, 15m, 1h, 4h.
...
def get_focused_configs():
    """15 configuraciones de máxima precisión cuantitativa para criptoactivos."""
    return [
...
def certifier_worker(ds_tuple):
    data_file, m_file, symbol, timeframe, mdata = ds_tuple
    certified_results = []
    ...
    for cfg_idx, (arch, ema_f, ema_s, rsi_p, rsi_l, rsi_s, sl_atr, tp_atr, risk_p, py_tiers, exit_fam, vol_filt, brk_lk) in enumerate(search_configs, 1):
        strat_id = f"UR_ULTRA_{symbol.upper()}_{timeframe.upper()}_fc{cfg_idx}"
...
def main():
    print("==========================================================================", flush=True)
    print("⚡ FAST CERTIFIER 11/11 ULTRA CRYPTO (SOL, XRP, BNB, AVAX, LINK, DOGE, BTC, ETH, SUI)", flush=True)
    print("==========================================================================", flush=True)
    ...
    print(f"📊 Datasets Seleccionados: {len(dataset_list)}", flush=True)

    all_certified = []

    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(certifier_worker, ds): ds for ds in dataset_list}
            ...
            try:
                ...
                if res_list:
                    ...
                    for item in res_list:
                        ...
                        print(f"🎯 CERTIFICADA 11/11: {s_id} | {sym} {tf} | PF={pf:.2f} | DD={dd:.2f}% | Trades={tr} | ROI/m={roi}%", flush=True)
                else:
                    print(f"ℹ️ Dataset {sym} ({tf}): 0 certificadas en este grid", flush=True)
            except Exception as e:
                print(f"❌ Worker error on {sym} ({tf}): {e}", flush=True)

    print("==========================================================================", flush=True)
    print(f"🏁 COMPLETADO. TOTAL ESTRATEGIAS CERTIFICADAS ULTRA CRYPTO 11/11: {len(all_certified)}", flush=True)
    print("==========================================================================", flush=True)

    for item in all_certified:
        print(f"  🏆 {item['strategy_id']} | Asset: {item['symbol']} {item['timeframe']} | PF={item['pf_oos']:.2f} | DD={item['max_dd_oos']:.2f}% | Trades={item['trades_oos']} | ROI/m={item['monthly_roi_pct']}% | Hash={item['canonical_hash'][:16]}...", flush=True)
